...
- **Graph full_scan is destructive, not incremental** — **medium (upgraded 2026-07-21)** — every scan (now scheduled 6h) clears and re-upserts all nodes; `body_hash` is stored but unused for skip. Embedding wipe fixed by DEF-066 snapshot/restore, but a second harmful effect is now confirmed: **nodes created directly via `graph_create_node`/`graph_decide` (spec-stage proposals, seams, decisions with no backing doc) are destroyed by every scan.** Observed live 2026-07-21 on mac-air: `proposal:memory-token-self-heal`, `proposal:muninn-vps-reharden`, and `proposal:data-driven-tool-grants-skilldag` (filed that morning, referenced by the merged handoff doc PR #339) are gone from both Macs' graphs, and `proposal:db-catalog-query-tool` + its 4 seam nodes + decision were wiped within ~90 minutes of filing. Until scans preserve operator/agent-authored node kinds (or snapshot/restore them like embeddings), graph-filed proposals MUST also land as frontmatter docs to survive — anything filed graph-only is silently lost within 6h.
