...
the architect-charter cron (autopoiesis A4); A4 has not landed as code/config
in this repo (see M3 row and disposition), so M3 shipped as a standalone
`memory.delta_digest` tool any role's toolset can call today, with the
charter-integration instruction named for whenever A4 lands. The LifeGraph
...
`codex/memory-m3-delta-digest`; see the M3 row above for what shipped, the
`evolved`-category gap, and the A4-charter delivery gap (Autopoiesis Slice A4
is unstarted in this repo — `ARCHITECTURE_STATUS.md` confirms — so the digest
ships as a standalone philote tool rather than an extension of a charter that
does not exist as code/config here yet). **Standing Rule 4 honest accounting:**
M3 landed the *pull* half only — the digest surface and the callable tool
exist and any role's toolset can invoke them on demand. The *push* half
("the digest pushes deltas daily") is not achieved: nothing calls
`memory.delta_digest` on a schedule today, because the only mechanism that
would (A4's daily cron) does not exist in-repo. Rule 4 ("both exist or
neither matters") is therefore not yet satisfied end-to-end — this is
deferred to whoever builds A4, not silently implied by "M3 landed." 
