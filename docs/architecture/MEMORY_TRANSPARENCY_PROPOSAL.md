...
| M3 `memory-delta-digest` | **Landed** (`codex/memory-m3-delta-digest`): on-demand per-hotel digest of what a trailing window (default 24h) remembered, forgot, and found contradictory, plus the latest `memory.hygiene` (M4) last-run marker. Collection lives in `crates/aiua/src/memory_delta_digest.rs`, reusing M4's REST wrappers (`memory_hygiene::fetch_engrams`, `fetch_contradiction_findings`) rather than duplicating them, plus two endpoint wrappers M4 didn't need: `GET /api/deleted` (soft-deleted items, client-side windowed since the endpoint has no `since`/`before` filter) and a bounded per-id `GET /api/engrams/{id}` detail fetch that extracts the M1 `ProvenanceEnvelope` from `metadata.provenance` for a top-5-per-category sample of "notable" lines (full-window enrichment would be an unbounded N+1). Exposed as a `memory.delta_digest` philote tool (`crates/philote/src/catalog.rs` + `memory_integration.rs`, dispatched from `tool_exec.rs`), reached over IPC via the same `GetConfig("__memory_delta_digest__[:hours]")` synthetic-key pattern Autopoiesis Slice A9's `__autonomy_status__` established — no new `IpcRequest` variant needed. Read-only: no autonomy grant is consulted (a digest renders existing state; it does not write). **Delivery reality gap, RESOLVED 2026-07-20 (see disposition):** at M3 landing time, Autopoiesis A4 (`aria-architect-charter`) had not landed as code/config, so there was no in-repo charter/prompt to extend. A4 has since landed (`codex/autopoiesis-a4-architect-charter`) and its role manifest calls this tool first, before composing the daily brief — see the disposition section below for the "push half" status. **Named gap inside the digest itself:** `evolved` is always `0` — MuninnDB REST has no queryable evolution history (`POST /api/engrams/{id}/evolve` is write-only), so this category cannot be enumerated until a MuninnDB REST addition exists; the digest states this explicitly rather than omitting the category or fabricating a count. | S–M | test-green (13 new unit tests in `memory_delta_digest.rs` — categorization, provenance parsing from fixture JSON, rendering — plus a philote catalog/skill-projection registration test); no watched-live digest review by an operator yet |
...
primitives) but its filings get richer once M1 lands. M3 was specced to ride
the architect-charter cron (autopoiesis A4); A4 had not landed as code/config
in this repo when M3 shipped, so M3 shipped first as a standalone
`memory.delta_digest` tool any role's toolset can call directly. A4 has since
landed (`codex/autopoiesis-a4-architect-charter`, see M3 row and
disposition) and now calls it on the daily charter cron. The LifeGraph
retrieval lane's Slice 4 (Muninn provenance) is M1's direct ancestor — build
on it, don't duplicate it.
...
rather than cache-derived.

M3 (`memory-delta-digest`) **landed** 2026-07-11 on
`codex/memory-m3-delta-digest`; see the M3 row above for what shipped and the
`evolved`-category gap. At the time M3 landed, Autopoiesis Slice A4 did not
exist as code/config in this repo, so the digest shipped as a standalone
philote tool rather than an extension of a charter that didn't exist yet.

**Update (2026-07-20, `codex/autopoiesis-a4-architect-charter`): the push
half is wired.** A4 landed the daily architect-charter role + cron
(`crates/aiua/src/architect_charter.rs`), and its role manifest
(`DEFAULT_CHARTER_MANIFEST`) instructs the steward to call
`memory.delta_digest` first, before composing the morning brief — exactly
the instruction this row named for whoever built A4. **Standing Rule 4
honest accounting, updated:** the *mechanism* for "the digest pushes deltas
daily" now exists end-to-end (daily cron → role turn → mandatory
`memory.delta_digest` call → brief), but no hotel has
`PHILOTIC_ARCHITECT_CHARTER_ENABLED` set yet, so nothing has actually fired
it on a schedule in production. This is `test-green` on the push mechanism,
not yet a live-observed push — the next seam is enabling the charter on one
hotel and watching the first scheduled digest call land in a real brief.
