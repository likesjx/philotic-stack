...
- [x] extended `just mcp-client-uat live` with token-backed `context.capture` and `life.recall` positive-path calls that skip safely when live tokens are absent
