...
| **R2** Membrane standby and push stand-down | **watched-live 2026-09-22.** Hotel pushes `IpcResponse::TransportHomeChanged` to local guests on a local `transport.set_home` and on a gossiped record applied by `placement_sync`; the Telegram seat has an explicit `Standby` state, stops polling at once when the home moves away, releases the lease on the next tick, and re-probes on the next 20 s lease tick when the home moves here. Discord/desktop/MCP standby pending | G7 | **watched-live ✓ (2026-09-22): Beacon's Telegram token flipped vps-jane → mac-jane; mac-jane's seat acquired the poll lease in 35 s (IPC-confirmed); operator sent a real Telegram message during the flip window and Beacon answered correctly, no drop, no misroute.** Took 5 defects to reach (DEF-107/143/157/168/169/177/178/179) — see `relocation_ceremony.md` for the full chain |
...
| **R7** Sealed secret transfer | when the transport moves with the role, its secret travels sealed: the target mints a single-use X25519 key at STANDBY; the origin seals with an ephemeral key, HKDF salted with the per-pair mesh key, AES-256-GCM with the full context (ceremony, STANDBY request, origin, target, agent, role, config key, kind, ACL, expiry) as AAD; release is authorized by the origin's own ceremony record only, to the STANDBY reply its target sent (DEF-170); the target stores it under its own vault key with the same non-empty ACL; the origin retires its copy after SWITCH and a marker stops `aiua load` re-seeding it. Test-green 2026-09-18; needs DEF-174 (fixed) and DEF-176 (tokens in the vault) | G4 | watched-live: Bjork's Telegram moves mac-jane → vps-jane with no operator token handling |
