...
- projected identity sync now rides the durable mesh event ledger as `ProjectedUserIdentitySync`, so the ghost mirror survives across hotels through the same selective shared-state plane we already use for other compact coordination slices instead of relying on whole-graph replication folklore
- local-first onboarding and OIDC resolution now consult that propagated ghost mirror: exact provider-subject matches reuse the existing principal directly, and unique verified email matches can adopt that projected principal instead of inventing a fresh hotel-local identity every time a new hotel meets the same human
