...
- runtime skill administration is hotel-owned and audited: typed LifeGraph SkillPatch confirmation releases an exact `skill.register_batch` compiler payload, role assignments live on `RoleIncarnationRecord` rather than shared profiles, and governed runtime skill records converge across hotels while role grants remain local
...
| Governed workflow skills | abstract skills persist full SkillDAG definitions and lifecycle/audit provenance; `skill.register` and typed `skill.register_batch` are hotel-owned mutation surfaces; confirmed typed LifeGraph SkillPatch records compile down without putting the runtime hot path behind Memgraph; assign/revoke grants are scoped to a role incarnation; governed runtime registrations converge between hotels over reliable authenticated transport | fleet rollout plus Beacon conversational UAT for the typed batch path; on-demand skill writers, data-driven turn relevance, profile administration, richer rollback, and real `SpawnSubagent` result routing remain open |
...
| Mesh-visible state placement | current local authorities mostly live in hotel runtime state, SQLite, or file-backed records; governed dynamic skill definitions are now explicitly mesh-visible, while compiled seeds and role assignments retain distinct release/local authority | classify the remaining state families; dynamic abstract-tool/profile convergence is still open and must not be inferred from skill convergence |
...
| Tooling and execution | partially implemented | [TOOL_ASSEMBLY_EXECUTION_PROPOSAL.md](/Users/jaredlikes/code/philotic-stack/docs/architecture/TOOL_ASSEMBLY_EXECUTION_PROPOSAL.md), [DATA_DRIVEN_TOOL_GRANTS_PROPOSAL.md](/Users/jaredlikes/code/philotic-stack/docs/architecture/DATA_DRIVEN_TOOL_GRANTS_PROPOSAL.md), and [MODEL_CONTROLLER_PROPOSAL.md](/Users/jaredlikes/code/philotic-stack/docs/architecture/MODEL_CONTROLLER_PROPOSAL.md) | role-scoped skill grants and typed SkillPatch compile-down are test-green; next pressure is fleet/Beacon UAT, dynamic tool/profile administration, broader structured failures, and embedding support |
