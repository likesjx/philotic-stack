...
| Model graph catalog | `model-router` currently owns provider execution, while `model.manager.list@1` projects node-advertised `model_ref` strings and provider capability truth still mostly lives in provider-local `supports()` logic; a shared provider-neutral catalog schema is now being introduced as the canonical static metadata layer for capabilities, endpoint stems, models, and variants | project catalog metadata into `model.manager.list@1`, keep static model facts separate from live node availability, and reduce provider-folklore hardcoding without turning the catalog into a shadow router |
| Turn routing plan | media routing, cognitive routing, and voice synthesis currently exist as separate runtime decisions; the first compiled `TurnRoutingPlan` shape is now being introduced so one inbound turn can carry explicit ingress, cognition, and egress stage intent without pretending one model choice owns the whole flow | thread the compiled plan into real voice-turn execution and observability while keeping the agent as turn owner and avoiding a shadow voice-specific agent loop |
...
| --- | --- | --- | --- |
| Runtime and sessions | implemented, still evolving | [SESSION_LOOP_PROPOSAL.md](/Users/jaredlikes/code/philotic-stack/docs/architecture/SESSION_LOOP_PROPOSAL.md), [TURN_ROUTING_PLAN_PROPOSAL.md](/Users/jaredlikes/code/philotic-stack/docs/architecture/TURN_ROUTING_PLAN_PROPOSAL.md), and code in `aiua`, `philote`, `ansible-mesh-core` | session ownership semantics, compaction policy, bounded loop follow-through, role context-shift semantics, and staged turn-routing plans for voice/media ingress and egress |
...
| Tooling and execution | partially implemented | [TOOL_ASSEMBLY_EXECUTION_PROPOSAL.md](/Users/jaredlikes/code/philotic-stack/docs/architecture/TOOL_ASSEMBLY_EXECUTION_PROPOSAL.md), [MODEL_CONTROLLER_PROPOSAL.md](/Users/jaredlikes/code/philotic-stack/docs/architecture/MODEL_CONTROLLER_PROPOSAL.md), and [MODEL_GRAPH_CATALOG_PROPOSAL.md](/Users/jaredlikes/code/philotic-stack/docs/architecture/MODEL_GRAPH_CATALOG_PROPOSAL.md) | structured model envelope and initial `request_class` routing are now smoke-green for the current cognitive path; next pressure is broader structured failures, embedding support, role-scoped toolsets, and a shared model graph/catalog that separates static model facts from live routing truth |
