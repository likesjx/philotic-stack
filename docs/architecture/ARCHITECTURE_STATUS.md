...
> **Status:** Transitional Snapshot | **Last Updated:** 2026-07-25
...
| Outbound integration and egress | canonical bindings, SkillDAG dependency projection, owner/grant policy, local/preferred/required/deny placement, vault-only credentials, the bounded HTTP runner, success/failure audit, operator management, and MCP-over-HTTP delegation are implemented; both outbound binaries are installed on `mbp-jane` and `vps-jane`; 33 production direct-client files now have machine-checked dispositions; the hotel-owned OpenRouter model-catalog sync is the first governed general-API migration and is watched-live-green from the launchd-managed MBP caller through `vps-jane-aiua-01`, with HTTP 200, bounded content-free audit, and a 342-model source-hotel projection; a separate watched two-hotel run proved required `vps-jane` execution, target-local credential resolution and durable audit, and response return to `mbp-jane` | remove the Philote direct catalog fallback, define credential-safe auth egress, and preserve explicit model/provider, communication, local-resource, mesh, and artifact exceptions while migration continues |
