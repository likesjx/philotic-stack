...
| Self-improvement loop (L5 prompt-guard + L1 skills.distill **implemented, watched-live-green mac-jane 2026-09-04**; L2–L7 proposed) | Borrow the mechanical half of Hermes Agent v0.21's learning loop under Philotic's existing gates: a turn-close distill whisper that can only produce `Draft` skills or Muninn writes (amends Autopoiesis A7), a provenance-scoped nightly skill curator that suspends/deprecates but never deletes, `skill.patch` as a policy-tier refinement with a durable pending queue surfaced in philotic-web, a fail-loud capped standing-notes block, a `prompt-guard` scan on everything rendered back into a prompt, cron `continuity` scratch, and FTS5 `session.search` | [SELF_IMPROVEMENT_LOOP_PROPOSAL.md](SELF_IMPROVEMENT_LOOP_PROPOSAL.md) — proposed |
| Procedural graphs (P0–P4 **implemented, test-green 2026-09-12** on `codex/procedural-graphs`; watched-live pending; P5 deferred) | Adopt Google's Procedural Graphs (arXiv 2609.09153) on the organs Philotic already has: `ProcedureGraphRecord` (tool-bound nodes, typed edges carrying condition/guidance/pitfalls) beside `abstract_skill` in the hotel context graph; a `procedure_run` ledger written from terminal plan evals instead of discarding `PlanEvalOutcome`; deterministic localized guidance (the active node's out-edges) appended to `reentry_hint` and `plan_continuation_brief`; harness plans seeded from a procedure's `LeadsTo` backbone; and a fourth distill trigger that contrasts a failed with a successful run of the same procedure, whose `procedure.patch` is approved into a live trial window and accepted only if the candidate version's grounded score holds — rejected patches are kept as negative evidence | [PROCEDURAL_GRAPHS_PROPOSAL.md](PROCEDURAL_GRAPHS_PROPOSAL.md) — accepted-current-slice, P0–P4 test-green |
| Reflexive Life Graph (**proposed 2026-09-15**; slice 1 = typed properties) | A skill that writes to the LifeGraph is a graph-bound procedure: `ontology_scope` (labels/edges/typed properties it owns, validated at register), `invariants` the runner executes read-only (declarative first, Cypher behind the wall) whose violations seed the gardening reflex, a bound `ProcedureGraphRecord` with deterministic ids, and structural `triggers`; underneath, a typed `properties` slot on the observe contract and the worker tool loop (DEF-129). Evidence: two days of repertoire drift caught by hand (sections on the wrong parent, a piece created twice, difficulty as prose). | [REFLEXIVE_LIFE_GRAPH_PROPOSAL.md](/Users/jaredlikes/code/philotic-stack/docs/architecture/REFLEXIVE_LIFE_GRAPH_PROPOSAL.md) |
| Graph doors and Life as core | Two layers behind one compiled-in Cypher wall (`cypher-guard`): graph-addressed Memgraph doors `graph.query` / `graph.analyze` / `graph.mutate` / `graph.admin` with a graph registry mapping ids to per-class containers (`life`, `agents`, `sandbox`), and the `life.*` tools as ontology-aware wrappers pinning `graph=life`. 100% of Cypher and MAGE exposed, every mutation through the writing door (dry run, provenance, audit, reversal hint, ConfirmFirst), admin operator-only. Skills gain a `guidance` field to carry their Cypher | [GRAPH_DOORS_AND_LIFE_CORE_PROPOSAL.md](GRAPH_DOORS_AND_LIFE_CORE_PROPOSAL.md) — proposed |
| Desktop generative surfaces (proposed 2026-09-14) | Philotes author operator interfaces as **data, never code**: A2UI v0.9 surfaces against a Philotic-owned catalog (`philotic.desktop.v1`), validated at the hotel and again in the desktop, carried as `turn_event{event:"ui_surface"}` on the existing routed operator-chat `/ws` stream and rendered by a hand-written vanilla renderer in `jaredlikes-desktop`; actions return as structured `ui_action` turns through `SendOperatorChatTurn` with a Philotic `action_id` (A2UI v0.9 has none), so the Telegram numbered approval card and a desktop approval surface resolve one approval record. AG-UI SDKs not adopted; frames stay AG-UI-mappable for a deferred SSE adapter. | [DESKTOP_GENERATIVE_SURFACES_PROPOSAL.md](/Users/jaredlikes/code/philotic-stack/docs/architecture/DESKTOP_GENERATIVE_SURFACES_PROPOSAL.md) |
...
| Membrane transport home | Telegram, Discord, MCP, and desktop now have separate membrane implementations or surfaces, and `role.set_home` already proves role placement is distinct from transport ownership; graph-owned `membrane_transport_home` records now exist with `transport.set_home` IPC/tool wiring, and Telegram lease acquire/renew enforces active-home records before falling back to legacy authority/delegated-hotels behavior Runtime placement is now graph truth that survives a restart and travels the mesh: `RoleIncarnationRecord.placement_updated_unix` and `MembraneTransportHomeRecord.updated_unix` stamp every `role.set_home` / `transport.set_home`, `seed_orchestrator_roles` preserves `home_node` on reseed (DEF-106), and runtime-stamped role homes + transport homes ride `HotelStateSync` where peers apply them last-writer-wins (`placement_sync`, half of DEF-107). A home change is pushed to every local guest at once as `IpcResponse::TransportHomeChanged` (local `transport.set_home` and gossip-applied records alike); the Telegram seat has an explicit `Standby` stand-down, stops polling immediately when the home moves away, releases its lease on the next tick, and re-probes on the next lease tick when the home moves here (DEF-107 closed, Relocation Ceremony R2) | extend the explicit standby state and push-time release from the Telegram seat to Discord/desktop/MCP membranes, surface seat standby in `hotel.status`, add desktop/operator-target parity for `transport.set_home`, and extend the graph-truth reseed guard from orchestrator homes to guest placement and transport-home seeds |
...
| Blob and execution plane perimeter | `HotelPerimeterService` is confirmed observational-only (no enforcement); the blob HTTP listener was found bound `0.0.0.0` with zero authentication while declaring itself loopback-only to the perimeter snapshot — a live, publicly-reachable anonymous write endpoint, verified off-tailnet on `vps-jane` 2026-08-31; the bind/declaration fix is in flight (PR #479, DEF-104) | restore authenticated cross-hotel blob transfer (the fix as written breaks the real `agent.deploy_bundle` migration path, which carries plaintext vault secrets — see BLOB_EXECUTION_PERIMETER_HARDENING_PROPOSAL.md), harden the execution port's unauthenticated connection layer, and close the firewall-parity gap (the intended `ufw` rule silently no-ops on hosts without `ufw` installed) |
...
| Mesh and placement | partially implemented | [INTER_HOTEL_ROUTING_PROPOSAL.md](/Users/jaredlikes/code/philotic-stack/docs/architecture/INTER_HOTEL_ROUTING_PROPOSAL.md), [MESH_VISIBILITY_AND_STATE_PLACEMENT_PROPOSAL.md](/Users/jaredlikes/code/philotic-stack/docs/architecture/MESH_VISIBILITY_AND_STATE_PLACEMENT_PROPOSAL.md), [MESH_PKI_HOTEL_IDENTITY_PROPOSAL.md](/Users/jaredlikes/code/philotic-stack/docs/architecture/MESH_PKI_HOTEL_IDENTITY_PROPOSAL.md), [BLOB_EXECUTION_PERIMETER_HARDENING_PROPOSAL.md](/Users/jaredlikes/code/philotic-stack/docs/architecture/BLOB_EXECUTION_PERIMETER_HARDENING_PROPOSAL.md) | placement policy, trust boundaries, overlay evolution, mesh-visible state classification, and closing the blob/execution listener perimeter gap |
