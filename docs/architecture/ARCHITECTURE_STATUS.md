...
- The `codex/notch-hover` follow-up adds near-notch hover opening and delayed pointer-exit collapse without application activation; the operator confirmed hover works. The refined panel is completely hidden at rest and grows from the camera bounds into a top-anchored shell. Content stays mounted to retain drafts, and pointer sampling continues while retracted, stopping only on explicit disable, display sleep or inactive user session. All 42 Mac tests pass; the new build's expanded layout and fully hidden resting state were inspected. New animation feel, focus and display/lock behavior await operator validation. See the [hover contract](../../apps/philotic-apple/COMPANION.md#mac-notch-hover--2026-09-15).
