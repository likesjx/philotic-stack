...
- Obsidian now has a derived metadata/lineage index for `Brain/Efforts/Ongoing`, narrow read and review-only proposal tools, explicit `authored_knowledge` context authority, non-blocking background refresh, and a fail-closed stdio MCP provisioner; targeted tests, isolated real-vault sync, and routed installed `mac-jane` status/search calls are green, while broader recall-quality and accepted-connection measurement remain incomplete
...
| Obsidian knowledge projection | `Brain/Efforts/Ongoing` has a test-green derived index with stable identity, rename lineage, tombstones, explicit live-body reads, metadata-only quick search, `authored_knowledge` context packets, and review-only create/patch/link proposals; the fail-closed seven-tool upstream is installed on `mac-jane`, connected, and watched-live-green for routed status/search calls | measure traceability, latency, duplicate rate, and accepted connections before widening vault scope |
| Creative learning flywheel | V006 is installed and verified on the live Memgraph; `Question → Idea → Experiment → Artifact → Learning` plus `Source` use governed proposed evidence, direct capture prefixes bypass the model, and the read-only `life.flywheel.brief` / `life.flywheel.review` surfaces are live. The updated `life-graph-runner` is installed and supervised on `vps-jane`; the production `mac-jane → vps-jane → Memgraph/ONNX → mac-jane` route is watched-live-green with a 982 ms capture. Deterministic daily and weekly jobs are active and first-fire-verified through Björk to Telegram | run the four-week `LifeGraph creative systems` pilot, measure artifacts, learning reuse, re-entry cost, and maintenance cost, then decide whether to widen domains; add timezone-aware cron scheduling if a fixed local wall-clock hour becomes important |
