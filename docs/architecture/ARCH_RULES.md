...
| `transported-agent-work-carries-graph-continuity` | `runtime-sessions` | `AGENT_RESOURCE_MODEL_PROPOSAL` | When a hotel transports agent-directed work and already knows the owning agent, it should carry a portable agent-graph snapshot or equivalent continuity signal so the receiving hotel can hydrate the agent graph before delivery. | hard | inter-hotel agent delivery, handoff payload shaping, agent graph hydration |
