...
| A4 `aria-architect-charter` | **Landed as code/config** (`codex/autopoiesis-a4-architect-charter`): config-shaped, per-hotel/per-agent registration of a daily "architect" steward — `crates/aiua/src/architect_charter.rs` extends the existing `role.create_or_update`/`RoleIncarnationRecord` + toolset-profile mechanism (not a parallel one) with an idempotent create-if-absent role seed and a daily `CronJob` targeting `role:{agent_id}:{role_name}`. The steward is a MODEL — the role manifest (`DEFAULT_CHARTER_MANIFEST`) is the prompt instructing it to call `memory.delta_digest` (Memory Transparency Slice M3 — wired in, not re-derived), sweep `phil heal list` / `docs/DEFECTS.md` / `phil graph status`+`phil graph seams` / `phil autonomy status` via `bash.exec`, author/update intel-graph proposals via `curl` against the REST API, and compose the morning brief. Delivery reuses the unmodified cron→role path (PR #80 heritage) and the DEF-051 reply-routing fix — neither touched by this slice. Opt-in per hotel via `PHILOTIC_ARCHITECT_CHARTER_ENABLED` + `PHILOTIC_ARCHITECT_CHARTER_AGENT` (no hardcoded agent); `CronTicker` re-checks this hotel's own opt-in **and** job-id match at fire time (stronger than M4's target-role-only re-check, since this charter's delivery is agent-specific, not hotel-symmetric — see the module's "Fire-time re-check" docs). **Delivery destination, traced not assumed:** `philote::runtime::handle_user_message` already defaults an inbound task's `final_reply_role` to `"membrane"`/local node when the task omits it, so a cron fire reaches the local membrane guest with no extra wiring — but `chat_id` is carried completely independently all the way to the outbound reply and is never inferred from `session_id` or `source`. `PHILOTIC_ARCHITECT_CHARTER_CHAT_ID` (+ optional `PHILOTIC_ARCHITECT_CHARTER_SOURCE`, default `telegram`) is therefore also required for the brief to actually land in an operator chat, baked into the registered job's payload at `ensure_scheduled` time the same way a manually `cron.register`-ed operator check-in (Beacon's Chronos) already does; `ensure_scheduled` warns loudly if it's unset rather than silently registering an undeliverable brief. **Reality gap named plainly:** unlike the toolset-profile seed (which propagates new grants via PR #298's reconciler), the seeded `role_manifest` text itself is create-if-absent only — improving `DEFAULT_CHARTER_MANIFEST` in a later release does not reach an already-materialized charter agent without an explicit `role.create_or_update` or a future A8b-driven reconciler. No hotel has this env-enabled yet, so this is `test-green`, not the `watched-live` this row originally asked for — first live daily-brief review is the next seam. | S | test-green (targeted `architect_charter` + `cron_ticker` suites); watched-live (first authored proposal + first daily-brief reviewed by operator) still pending |
...
- **A4 `aria-architect-charter`** — the live-DB-staged charters (Beacon on
  vps-jane, Aria on mbp-jane; Coach on mac-jane was in flight) predate this
  row and were never code/config in this repo — see the A4 row above for
  what landed as code/config on `codex/autopoiesis-a4-architect-charter`
  (2026-07-20): a generic, config-shaped role+cron mechanism any hotel can
  opt into via env vars, extending the existing `RoleIncarnationRecord` +
  toolset-profile machinery rather than the ad hoc live-DB patches. No hotel
  has the new mechanism enabled yet — the previously-staged live-DB charters
  are unaffected and keep running independently until an operator migrates
  them to this path.
