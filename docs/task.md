...
- [ ] Re-smoke after the DEF-110 fix: exactly one reply per call in the hotel log, `recall_note` answered by bjork's own memory (or one model turn when empty) — **smoke-green** target.
