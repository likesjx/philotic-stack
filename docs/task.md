...
- [ ] **Phase 2 (2026-09-16 audit)** — philote memory efficiency, sleep, repair. See proposal § Phase 2. Sequence M1 → M2 → M3 (one PR rebased on develop) → M4 → M5 → M6 → M7.
  ...
  - [ ] **M7** repair (operator window) — **in progress 2026-09-16**. Full inventory: mbp missing default 49 / Beacon 50 / Astrid 1; mac missing Beacon 19. Observer-only memories rescued to the Cortex (mbp 5, mac 174; skipped Cortex-deleted and empty ones). **mbp restored from a Cortex checkpoint and verified complete (0 missing across all vaults; recall live).** mac-jane: run `bash ~/.muninn/resync-mac.sh` (operator; daemon stop/swap is permission-gated). Original plan: enumerate observer-only memories → re-write to Cortex → Cortex checkpoint restore onto each observer (keeps ULIDs, reversible) → diff. Prevention: raise `max_log_backlog`, upstream applier contiguity guard + digest + snapshot wiring, linux rc1 Cortex build.
