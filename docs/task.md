...
- [ ] **Phase 2 (2026-09-16 audit)** — philote memory efficiency, sleep, repair. See proposal § Phase 2. Sequence M1 → M2 → M3 (one PR rebased on develop) → M4 → M5 → M6 → M7.
  ...
  - [ ] **Gap (found live)**: Björk (`agent-bjork-01`) has no `memory.remember` in its toolset, so M5's widened gate cannot fire for it; on a smoke turn the model claimed "I have taken this into my memory" with no tool call (plan gate caught it). Decide the grant (and audit other personas' grants).
  - [ ] **Gap (found at deploy)**: M6 sleep is deployed but dormant — `PHILOTIC_DREAM_SWEEP_ENABLED` has no pass-through in `ansible/roles/philotic_hotel/templates/philotic-hotel.service.j2`. Add a template var (default off), enable on vps-jane, read the first proposal-only `memory_sleep:last_run:vps-jane` before ever setting `PHILOTIC_MEMORY_SLEEP_MUTATE`.
  - [ ] **M7** repair (operator window): enumerate observer-only memories (mbp 5 philote + `default` TBD; mac 2 philote + ~179 `default`) → re-write to Cortex → Cortex checkpoint restore onto each observer (keeps ULIDs, reversible) → diff. Prevention: raise `max_log_backlog`, upstream applier contiguity guard + digest + snapshot wiring, linux rc1 Cortex build.
