...
- [x] **S6a** — Muninn admin observability (reporting). Landed (PR #466): `memory_report.rs` honest-sourcing model + `assemble_live_memory_report`; a safe **windowed** `count_recent_session_events_by_kind` (DEF-080-guard) sources recall effectiveness (completed/skipped); wired end-to-end as the admin-gated `memory.report` tool (`IpcRequest::GetMemoryReport` → aiua handler → philote tool_exec → catalog + local_agent_tools + runtime; granted on admin/orchestrator profiles, class-less so it doesn't leak via the `memory` class). test-green + clippy clean. **Follow-up (S6a extras):** multi-node vault counts + divergence + contradictions/soft-deleted + disk (currently honest `unavailable`).
...
- [x] S3 — deterministic operator-fact capture: high-precision `classify_operator_fact` (imperative/`my X is Y`/preference/name; never on questions), targeting the `user` scope so captures reach the shared store; importance 0.4. test-green (PR #466). **Remaining:** attend-hook wiring (fire when `memory_candidate` is None) — deferred until live-validated to not over-capture.
- [x] S4 — promotion criterion + posture gate: `PromotableSignals::is_promotable` (high-importance, durable-type, non-sensitive, self_/user_ source) + `promotion_action` (ProposalOnly/ConfirmFirst propose only; only AutoWithAudit writes to `fleet_knowledge`). test-green (PR #466). **Remaining:** sweep integration (fetch candidates + gated write) + the consolidate/dedup/archive mutations — deferred until live-validated before mutating the store.
  - **S4 input landed in muninndb:** `feat/self-knowledge-surface` merged into `integration/observer-replication` (`c7ec040`, binary `v0.11.2-rc1`). Opt-in `self_knowledge:true` recall returns `contradicts_ids` (model-free contradiction pass over the returned set) + always-on `superseded_by` staleness. **Philote wiring DONE (PR #466):** memory-core sends `self_knowledge:true` on every activate, parses `contradicts_ids`/`superseded_by`/`possibly_superseded_by` into `metadata.annotations`; `project_recalled_memory` renders explicit ⚠ CONTRADICTS / ⚠ STALE lines. Consuming them as cultivation mutations (evolve/resolve/archive) is the S4 sweep-integration step.
- [ ] **S6b** — reporting fields needing a **muninndb** API (replication lag, peer/streamer state, backlog depth). Cross-repo (Go project at ~/code/muninndb) + deploy; out of scope for an in-repo code slice.
- [ ] **Phase 2 (2026-09-16 audit)** — philote memory efficiency, sleep, repair. See proposal § Phase 2. Sequence M1 → M2 → M3 (one PR rebased on develop) → M4 → M5 → M6 → M7.
  - [x] **M1** (test-green, PR #466) recall query + relevance gate: drop `role:` prefix (one memory dominates 93% of Beacon recalls), follow-up/continuation seeding, honour `relevance_band`, 0–5 items, skip empty/failing vaults, partial-401 heal.
  - [x] **M2** (test-green) projection budget: slim item format, ~300-char preamble, fix ns timestamps, drop duplicate annotations + superseded items, LifeGraph own lane, stable prefix for prompt caching (today 97–99.5% of blocks truncated and re-sent on 73–89% of later calls).
  - [x] **M3** (test-green; also fixed `memory.report` counting the wrong event field — it always read 0) engine hygiene + honest telemetry: shared engine, 1.5 s timeout, `memory_auto_recall_failed` event, band/score/latency in events, fix `memory.report` hit rate (reads ~100% vs real 91.6%), access feedback to Muninn.
  - [x] **M4** (test-green; Cortex hotel provisions tokens for forwarded self vaults on first sight; token heal for observer READS still cannot mint) write routing: every philote write + token mint targets the Cortex via `memory.write_forward` (Mac Attend-hook writes get 421 today; last Björk memory 07-11); durable retry queue (S2 remainder).
  - [x] **M5** (test-green; S3 capture wired, 10/agent/day) remember volume/quality: replace keyword gate on `memory.remember`, wire S3 capture after M4, collision-safe concept slugs, keep probes/self-heal out of persona vaults.
  - [x] **M6** (test-green; deterministic dedupe + probe forget, proposal-only unless `PHILOTIC_MEMORY_SLEEP_MUTATE`; removed from shutdown drain. Ops at deploy: `PHILOTIC_DREAM_SWEEP_ENABLED` on vps, `--metrics-addr` on nodes) sleep on the Cortex hotel only: contradictions, consolidate (fix `dream.rs` vault discovery), expire dated events, enrichment backfill, tombstone purge, cross-node diff alert, `--metrics-addr` on all nodes. Never the `muninn dream` CLI.
  - [x] **Deploy** 2026-09-16: PR #466 → develop `98e742b0`, deployed vps-jane (CI artifact), mac-jane, mbp-jane. **Live-verified on mac-jane only**: auto-recall query without role prefix, band gate (weak dropped), 62 ms; forwarded `self_agent-bjork-01` write → vps provisioned a token on first sight → Cortex write → replicated to both observers → forwarded forget soft-deleted it everywhere. vps/mbp: binaries verified by literal strings; natural-traffic evidence and a `memory.report` failure-count read still owed.
  - [x] **Memory tools for all philotes** (operator decision 2026-09-16). Root cause of the Björk smoke failure was NOT a missing grant (orchestrator grants both): a task with no `agent_id` created an unbound session whose snapshot had no role, so the turn ran on the always-on minimum toolset. Fixed: restored sessions with no role and no toolset now run default-role activation; `codex`/`research`/`utility`/`scheduler`/`virtuoso` profiles grant `memory.recall` + `memory.remember` (reaches hotels via seed reconcile on restart); `memory.remember` is no longer behind a write-intent keyword gate; smoke driver sends `agent_id`. `companion` (vixen) stays tool-free by design (its model has no tool endpoint).
  - [x] **M6 sleep enabled** on vps-jane (PR #542; live drop-in until the next ansible deploy). First on-demand proposal run: 3 vaults, 232 memories, 0 duplicates, 9 diagnostic (all genuine May test probes in Björk's vault), 0 contradictions, 4 tombstones. Mutate stays off pending operator. Diagnostic tag rule tightened after a false positive (a real procedure memory tagged `smoke`).
  - [x] **M7** repair — **done 2026-09-17**. Full inventory: mbp missing default 49 / Beacon 50 / Astrid 1; mac missing Beacon 19. Observer-only memories rescued to the Cortex first (mbp 5, mac 174; skipped memories deleted on the Cortex and empty ones). Both observers restored from fresh Cortex checkpoints by the operator (`~/.muninn/resync-mbp.sh`, `~/.muninn/resync-mac.sh`; rollback copies at `~/.muninn/data/pebble.pre-resync-*`) and **verified complete: 0 missing across all vaults on both observers**; recall live on mbp.
  - [x] **Follow-up from M7 — done 2026-09-17**: the Muninn admin credential now lives encrypted in each Mac hotel's vault (`muninn_admin_secret_ref`); `aiua load` prefers it and never persists `admin_password` (PR #547). Plaintext removed from `config:muninn` and `mesh-config.json` on both Macs; mbp's config load succeeds again. Also 2026-09-17: memory sleep mutation enabled on vps-jane (first run forgot exactly the 9 flagged probes), and pre-0.11.0/pre-resync Muninn backups deleted on both Macs.
  - [ ] **Prevention (M7)**: raise `max_log_backlog`, upstream an applier contiguity guard + per-vault digest + real snapshot wiring to muninndb, linux rc1 Cortex build.
  - [ ] Security: rotate Muninn admin credentials stored plaintext in mac-jane `config:muninn`.
- [ ] **S5** — replication repair (LIVE-OPS, needs operator): merge fix/observer-cache-invalidation (#869) + fix/replication-log-retention into muninndb develop; pin binaries (kill mac dev skew); repair observer apply + :8490; reconcile 179 mac-only up BEFORE any reseed. Restarts the live memory daemon + fleet deploy — not an autonomous background action.
...
- [ ] Implement [MEMBRANE_TRANSPORT_HOME_PROPOSAL.md](/Users/jaredlikes/code/philotic-stack/docs/architecture/MEMBRANE_TRANSPORT_HOME_PROPOSAL.md) as the transport-agnostic version of that placement policy:
  ...
  - [ ] make all membrane startup paths expose standby state before acting — Telegram seat done (Relocation Ceremony R2, `StandDownReason::Standby`, 2026-09-05); Discord / desktop / MCP membranes pending
...
- [ ] **Attachment evidence + batch ids (2026-09-17, `codex/media-evidence-carry`)** — operator review of bjork's morning ("Fuck! You didn't update the hymns from that image"). DEF-163 re-entries carry the refused draft so an attachment's reading survives the say-do gate; DEF-164 a caption-less attachment with an open operator request re-enters as INPUT instead of being described back; DEF-165 batch items get their ids synthesized. Watched-live gates: (a) a photo + instruction caption whose first reply is refused still writes what the photo says, not a recalled older value; (b) a caption-less photo after "send me X again" performs X; (c) a model-authored `life.observe.batch` with no packet_id lands on the FIRST attempt. Left over from the same review: duplicate practice logs for 09-16 night to be tidied by bjork (Beacon's `life:event:music_practice_20260916_tonight` vs bjork's `organ_practice_20260916_hymns` + `piano_practice_20260916_classical`); `properties.kind`/`notes` still undeclared for CreativeWork/Event (R1 typed properties needs the music keys declared); agent-coach's `life.recall` hung 301 s (watchdog evicted); Muninn token heal 421 for vault `user_likesjx` degrades recall.
...
- [x] Direct membrane surface (2026-08-12): `@LyraPhiloteBot` live — token under `agents.lyra.telegram` (allowed user `likesjx`), `membrane-gateway-lyra` holds the poll lease and is long-polling; voice policy set (ElevenLabs `Lily` / `eleven_v3`, captions + text fallback). Voice render itself is proven on Lyra's first spoken reply.
...
- [x] L1 `skills.distill` lane: three turn-close predicates at `deliver_text_reply`, lookaside distill whisper (`ParacrineRouting::Discard`, fixed tool allowlist), `Draft`-only output via `RegisterSkill.origin`, 3/day budget via `ConsumeAutonomyAction { filing: true }`, kill switch — **test-green** 2026-09-03; **watched-live 2026-09-04 on mac-jane** — user_correction trigger → grant + pending audit → whisper to own role → `DISTILL: nothing` → Discard, 3 s, nothing surfaced. Second drill (tool_count, 5 tools) exposed that the lookaside was offered no `skill.register` (Bjork incarnated as `architect`; on-demand gating) — fixed: whisper targets the agent's `orchestrator` by scoped routing key when the current role lacks `skill.register`, and distill turns project the allowlist bypassing keyword/on-demand gates. **WATCHED-LIVE-GREEN 2026-09-04 18:46 UTC**: after DEF-105 and the clean-context fix, a real six-tool turn produced Draft `music.weekly-practice-review` (`origin=distill:tool_count`, registered by `agent-bjork-01:orchestrator`, nothing surfaced) in 4 s.
- [x] L1 follow-up: distill lookaside turns now run on a clean context — no dialogue window, no recalled memory, no Muninn auto-recall, no LifeGraph prefetch/injection (2026-09-04; the persistent lookaside session was echoing its own earlier `DISTILL: nothing` and a refused register back into each new review).
...
- [x] P0 `procedure-graph-record`: `ansible-mesh-core/src/procedure.rs` (`ProcedureGraphRecord`, `validate()`, `linear_backbone()`/`backbone_from()`, `locate()` with predecessor disambiguation, `tool_overlap()`, `outcome-reflex` seed); node kinds `procedure` / `procedure_run` / `procedure_patch`; `GraphDomain` CRUD + fill-only `seed_procedure`; IPC `RegisterProcedure` (skill-admin gate, prompt-guard, agent origin → Draft, wire provenance → Operator, version bump) / `GetProcedure` / `ListProcedures` / `RecordProcedureRun` / `ListProcedureRuns`; boot seed; `effective_procedures` in session bindings carried by `merge_snapshot_bindings`; `phil procedure list|show|runs` — **test-green** 2026-09-12 (`codex/procedural-graphs`)
- [x] P1 `procedure-run-ledger`: `ActivePlan.procedure_id`; `philote/src/procedures.rs` attribution (stamped id, else Jaccard ≥ 0.5, deterministic tie-break) + `build_procedure_run`; `plan_followup_after_turn` stashes the row only on terminal branches (Complete / Blocked / budget or lifetime stop / continuation-disabled), never `Continue`; `deliver_text_reply` drains it and appends via `RecordProcedureRun` (5 s timeout, best-effort); hotel derives the score server-side — **test-green** 2026-09-12
- [x] P2 `procedure-localized-guidance`: `procedures.rs` localizes on the last matching tool call (entry node before the procedure starts, predecessor disambiguation), renders `Next/when/do/avoid` from the active node's out-edges (incoming pitfalls when the last call failed), 900-char cap, `PHILOTIC_DISABLE_PROCEDURE_GUIDANCE`; appended after `reentry_hint` in both prompt builders and to `plan_continuation_brief` (localized on the carryover's verified steps) — **test-green** 2026-09-12
- [x] P3 `procedure-seeded-plans`: `ProcedureEdge.branch` tag; `seed_plan_from_procedure` projects the chosen branch's backbone (`{target}` substituted, operator excerpt on step 1, `procedure_id` stamped); `seed_outcome_plan` uses it when a procedure with trigger `reports_an_outcome` is bound, literal fallback retained — **test-green** 2026-09-12 (all four existing seeding tests unchanged). Deferred: a `lifegraph.gardening` seed (the gardener's real shape needs reading on vps-jane before it is encoded).
- [x] P4 `procedure-refiner-gate`: `ProcedurePatchOp` / `apply_patch()` (copy, re-validate) / `ProcedurePatchRecord` (never deleted, `base_snapshot` for revert) / `decide_trial()`; IPC `ProposeProcedurePatch` (any registered guest, dry-run + prompt-guard, Pending, refused while on trial) / `ListProcedurePatches` / `DecideProcedurePatch` (skill-admin; approve → `v+1` with `trial_of` + Refiner provenance; reject retained); `evaluate_procedure_trials` on every `RecordProcedureRun` (accept clears the marker, reject reverts and records both means); lane `procedures.refine` 3/day + `PHILOTIC_AUTONOMY_DISABLE_PROCEDURES_REFINE`; philote tools `procedure.get|register|patch` (under `skill.authoring`, distill allowlist, lookaside filings skip approval), `DistillTrigger::ProcedureContrast` fired once per (failed, success) pair at the current version via the shared `emit_distill_whisper`, contrast prompt = triplets + both trajectories + last five rejected patches; distill YES path also registers a linear Draft procedure; `phil procedure patches|approve|reject` — **test-green** 2026-09-12. **Watched-live pending**: one real `Pending` patch from a Beacon plan failure on vps-jane, approved, trialled, decided without a deploy.
- [ ] P5 `procedure-generative-guidance` — deferred until P2 has a watched-live baseline
- [x] Deploy: mac-jane hotel already carried P0-P4 (prior deploy); vps-jane hotel already carried it too (02:54 UTC build), but its `phil` CLI was stale (Sep 7, predates `procedure` subcommand) — refreshed via `just vps-deploy-ci` 2026-09-16 (ansible ok=42 changed=2 failed=0). `phil procedure list` confirms `outcome-reflex` v1 (repo, 4 nodes/4 edges, `life.steward`/`reports_an_outcome`) live on **both** hotels — verified via `sudo -u philotic ... procedure list` on vps (socket is philotic-owned) and a direct socket override on mac-jane (installed `phil` predates the `procedure` subcommand; use the synced-checkout build).
- [ ] Watched-live remainder (NOT fabricated — needs a real operator outcome report): an Icebreaker-style report seeds a plan with `procedure_id` and lands one `procedure_run` row (`phil procedure runs outcome-reflex`) on either hotel; both currently show zero rows.
- [ ] P4 trial gate watched-live: needs a real failed-and-successful run pair of the same procedure at one version — cannot be forced, wait for it to occur naturally.
...
- [x] R1 Graph truth outlives seed — **test-green 2026-09-05** (`codex/relocation-r1`): `placement_updated_unix` / `updated_unix` stamps on both home record kinds; `seed_orchestrator_roles` and `role.configure` preserve `home_node` + stamp (DEF-106 fixed, regression test); runtime-stamped role homes + transport homes ride `HotelStateSync` (every chunk) and peers apply last-writer-wins via `ansible_mesh_core::placement_sync` (3 unit tests); `RoleHomeSet`/`TransportHomeSet` mark hotel state dirty. Zero-stamp (seed-only) records never travel. Guest-row placement fields deferred: `preserve_runtime_guest_activation` already keeps `is_active`. Smoke pending: restart origin hotel, home survives.
- [x] R2 Membrane standby + push stand-down — **test-green 2026-09-05** (`codex/relocation-r2`): `IpcResponse::TransportHomeChanged` push (marker field, declared before `TransportHomeSet`) from the hotel to every local guest on a local `transport.set_home` and on a gossiped record applied by `placement_sync` (beacon `placement_change_tx` → `MeshRuntimeContext` → broadcast); Telegram seat: explicit `StandDownReason::Standby` (entered on `LEASE_TRANSPORT_HOME_MISMATCH` via the captured denial code, or on a home-moved-away push), poll loop stopped at once, lease released on the next tick, home-moved-here push arms an immediate re-probe (next 20 s lease tick instead of 180 s). DEF-107 fixed. 3 seat tests + 1 IPC broadcast test + 1 untagged round-trip test. Discord/desktop/MCP membranes still lack the explicit standby state. Watched-live pending: Beacon token vps ↔ mac with no dropped message.
...
- [ ] R5 Continuity transfer — **test-green 2026-09-18** (`codex/relocation-continuity`, prerequisite DEF-167 restore fidelity PR #559). CONTINUITY exports the moving role's session checkpoints (front door: base keys + session index; a role: its `:{role}` keys), their `session` rows, and the agent identity as the last read before SWITCH, ships them inline as `ContinuityImport` over the HMAC-signed execution plane (no blob plane needed; signed, not encrypted, so no vault entries), and SWITCH waits for the target's `ContinuityAck`. Import rewrites `delivery_*` + the `{hotel}:` guest prefix, moves `final_reply_to` only when the transport moves, seeds (never overwrites) the identity, and is idempotent per ceremony. Targets advertise `supports_continuity` in the STANDBY reply; an older target gets the old degraded move. Open: (a) drain contract — a turn the origin finishes between export and SWITCH, and the origin philote's in-memory queued tasks, are not carried; (b) a target process that already has the session loaded in memory will not re-read it. Smoke pending: parked turn survives a move mac-jane ↔ vps-jane.
...
- [ ] R7 Sealed secret transfer — **test-green 2026-09-18** (`codex/sealed-secret-transfer`, after #563 sender binding + operator authority and #564 Telegram tokens in the vault). A moving transport's token is sealed to a single-use key the target minted at STANDBY (X25519 + HKDF over the per-pair key + AES-256-GCM, context as AAD), released only by the origin's own ceremony record to the target that sent the STANDBY reply, stored on the target with the same membrane-only ACL; the origin retires its copy and marks it so a re-seed can't make a second holder. DEF-174 closed as prerequisite. Open: DEF-171 (who may move a home), DEF-173 (local socket identity), DEF-175. Watched-live pending: Bjork mac-jane → vps-jane.
...
- [x] Live LifeGraph cleanup on vps Memgraph — APPLIED by the operator 2026-09-11 in three passes (mgconsole on stdin stops silently after a multi-line statement block; the committed script is now one statement per line). Also found and removed five older DEF-117 strays; `strays_remaining = 0`.
- [x] Deployed vps-jane (PR #492 → develop dd586bb9, `just vps-deploy-ci`, hotel restart 14:23 UTC, fix strings verified in `philote` + `life-graph-runner`) and mac-jane (`deploy-mac-jane.sh` + bootout/bootstrap, 8 philotes on the new inode).
- [x] First watched-live turn after deploy (18:23 UTC, "I gave my icebreaker speech…") FAILED a different way: conversational gate projected zero tools for a lived-fact report about a recalled loop, and Beacon claimed "I have logged this historic event" with no tool call → DEF-118, fixed on `codex/philote-lived-fact-tools` (recalled-context override of the gate + past-tense claim detection in the say-do gate). Operator rule recorded: completed items are RESOLVED (or turned into an outcome event), never deleted.
- [x] Plan contract with teeth (DEF-119, `codex/philote-plan-contract`): outcome reflex seeds observe+commit plans for reported outcomes that name a recalled loop; plan-worthy statements keep tools; plan gate re-enters a text-only reply once. Watched-live: re-tell Beacon the Icebreaker outcome → expect `plan_seeded`, `life.observe`, `life.commit` on `life:open_loop:toastmasters_icebreaker_speech_20260906`, `plan_eval verdict=complete`, and a reply that reports what was written.
- [x] Second live turn (20:46 UTC) on the plan-contract build: plan gate fired, but the stale prefetch cache meant no loop was in context so nothing seeded, and the reply "we already successfully initialized and processed…" dodged the claim detector — and was backed by a Muninn engram the 18:23 false claim had created (DEF-120). Fixed on `codex/philote-outcome-reflex-v2`: recall-first three-step seed when no loop is in context; adverb-stripping claim detector with we/was/were subjects; unbacked-claim turns write no memory. Poisoned engram `toastmasters-icebreaker-delivered` to be forgotten by hand in Beacon's vault.
- [x] Watched-live 2026-09-12 11:00 UTC brief (DEF-113 build): stamps ✓, status trailer ✓, explicit `life.recall` returned packets ✓ — but still 4 messages: a real enumeration (four Person nodes in one step) stayed non-atomic while the model re-ran the observes instead of splitting → DEF-121, fixed on `codex/philote-plan-autosplit` (harness splits bundled steps per item in `set_active_plan`). 12:28 UTC: say-do check fired on "committed this to my operational context" and the correction trailer shipped; the follow-ups wrote real nodes. Graph hygiene from those turns for the operator: retire `life:commitment:iphone_18_pro_delivery` (operator said it is an Event, not a Commitment); `life:person:daxton` duplicates `life:person:daxton_thomas_likes`.
- [x] Watched-live 2026-09-12 16:47–17:01 UTC on the DEF-121 build: **outcome reflex proven** ("Daxton has been reinstated…" → `plan_seeded` → observe + commit → `plan_eval complete`; graph verified: event confirmed, monitoring commitment + investigation loop resolved); say-do check fired once and the model then recalled; people and places were really written. Found DEF-122 (commit `packet_id` retries) and DEF-123 (re-observe never updates `claim_summary` — Home address lost), fixed on `codex/lifegraph-observe-update-commit-packet`. Still open: a turn WITH tool calls can over-claim items it did not write (church turn claimed an encounter event and a Mint note; only the two people were observed) — needs a post-hoc claim audit (cited `life:` ids vs this turn's tool history) rather than the zero-tool gate.
- [x] Claim audit (DEF-125, `codex/philote-claim-audit`): cited `life:` ids vs this turn's writes/context → correction trailer + `claim_audit` event.
- [x] `lifegraph.gardener` slice 1 (`codex/lifegraph-gardener`): runner tools `life.audit` (read-only graph science over a full export — union-find components, giant/islands, live orphans, degree + PageRank hubs, exact + embedding-cosine duplicates with confirmed-else-newest keeper, past-due/untouched loops, missing dates/observed_at, bare-numeric/missing ids, unknown rel types, health_score, `suggested_actions` + `needs_judgment`) and `life.tidy` (one governed action per call: retire_duplicate → SUPERSEDES, link, resolve, retire; never delete; `tidied_at/by/reason` stamps; confirmed needs operator_approved); catalog tool schemas + `lifegraph.gardener` skill (implied tools, relevance keywords), hotel seeds (abstract tools, skill record, on-demand for the profiles that carry `life.steward`, runner supported_tools), `skills/lifegraph-gardener/SKILL.md`; ansible pins `memgraph/memgraph-mage:3.10.1` (same version as the running 3.10.1) so MAGE procedures (community detection, node similarity) are available for the next slice. Watched-live: ask Beacon to "audit and garden the LifeGraph" → `life.audit` report, one `life.tidy` per action, second audit with a higher score; then she registers the daily cron herself.
- [x] Gardener watched-live 2026-09-14 20:34–20:49 UTC ("Audit and garden the LifeGraph."): `life.audit` ran (646 nodes, 247 edges, 100 live orphans, 440 components, giant 199, 13 duplicates, 25 suggested actions, health 0), one duplicate retired under its keeper with SUPERSEDES, three islands linked (Tailscale action→asset, workout routine→wellness role, gift-card loop→its next action); claim audit fired on a hyphen/underscore id mismatch. Gaps → slice 2 (`codex/lifegraph-gardener-reflex`): the model applied 1 of 25 actions then asked "if you would like…" (no plan declared) → **gardening reflex** seeds one `life.tidy` step per suggested action (≤12) + a closing `life.audit` when an audit result lands; the 20:47 reply never mentioned its four real writes → **write receipt** trailer lists uncited writes; the claim-audit correction on `…-blank-page` vs `…_blank_page` made the model create a second CreativeWork → ids compared normalized, wording softened ("recall it and cite the exact id"); health score saturated at 0 on 20 legacy custom edges → per-category caps, distinct unknown edge TYPES scored once with a `needs_judgment` line to register them via `life.patch.propose`; the model set `operator_approved: true` itself → philote drops it on a `retire` unless the operator's message reads as approval.
- [x] Gardener watched-live 2026-09-14 21:20–21:35 UTC on slice 2 ("Garden the LifeGraph."): the seeded 13-step plan ran — 7 tidy actions verified in-turn, receipts on the reply — then three defects: (1) the twelve retire_duplicate steps share every word, so the token verifier credited each tidy to the first unproven sibling; the "outstanding" real steps were re-run on the continuation and hit "matched nothing" (already retired) five times → stall detector killed the turn and its evidence; (2) the closing `life.audit` step was never called — three continuations replied "all 12 verified" and the plan blocked at 0/1 (the operator: "she keeps dropping the last step"); (3) a `{"display_text": …}` envelope with raw newlines reached Telegram verbatim. Slice 3 (`codex/lifegraph-gardener-closer`): **exact-id verification pass** (an id-bearing step is proven only by the call carrying all its ids; never falls through to token passes); **idempotent tidy** (a retry on an already-retired/resolved node returns `already_applied`, not an error); **harness closer** — on a continuation whose remaining steps are all read-only (`life.audit`), the harness issues the call itself (`plan_step_harness_run` event) so the measurement never depends on the model; **envelope unwrap** — a leaked structured reply is reduced to its `display_text` before the gates.
- [x] Slice 3 watched-live 2026-09-15 14:09–14:14 UTC on #515 ("Garden the LifeGraph"): audit 51 → 12 tidies verified with receipts (3 duplicates retired, 9 orphans anchored) → the harness closer ran `life.audit` on each continuation (health 58, orphans 87→76, dups 3→0) — and the plan still blocked at 12/13 because the closing step's words never appear in an argument-less audit call (DEF-131; the reply said "step(s) 13" and Beacon answered "there was no Step 13"). Slice 4 (`codex/philote-unique-tool-verify`): unique-tool verification pass; the seeding audit is fenced off as evidence (`plan_evidence_from`); outstanding steps are named in the status trailer; ids already in the dialogue are not re-audited as claims.
- [x] Slice 4 watched-live 2026-09-15 16:14–16:15 UTC on #516: audit 58 → 12 orphans anchored SCOPED_TO in one 43 s turn, receipts on the reply — and `plan_eval` said 13/13 complete with NO closing audit: the turn-end evaluator (`plan_followup_after_turn`) re-verifies from the full working history and the unique-tool pass credited the seeding audit. The reply's "Structural Impact & Health Score" section had no numbers (nothing to report). Slice 5 (`codex/philote-closing-audit-fence`): #516's order-blind unique-tool pass had already been narrowed to read-only tools with the DEF-130 call-order floor by #517 (#516 was merged with its test job pending and regressed two DEF-130 tests); the `plan_evidence_from` fence now also applies at the turn-end evaluator; regression test replays the live turn.
- [x] Second pass on #516, 2026-09-15 16:32–16:34 UTC: turn 1 ended 12/13 with "still working on step 13 (Re-run life.audit to measure the pass)", the continuation's harness closer ran the audit (59 → 61), plan_eval complete 13/13 — and three follow-ups (DEF-136): the model re-ran all 12 completed links (brief carried the call payloads; tools still offered after the plan was fully verified), the re-links came back `tidied` (MERGE could not tell matched from created) and were listed as writes, and the delta was never reported. Slice 5 additions: payload-free completed steps; no tools + "[Plan complete]" note once every step is verified; link idempotency (`created` flag → `already_applied`); receipt skips no-op retries; baseline `health_score N before` in the goal + harness-appended "📊 LifeGraph health: N ↑ M" trailer.
- [x] Slice 5 watched-live 2026-09-15 18:30 UTC: turn 1 audit 61 → 10 tidies, plan_eval 11/13 with steps 3 and 7 "outstanding" although both were called and tidied; in-turn the working state had already said "[Plan complete]" (no-tools re-entry fired as designed); continuation re-ran 3 links (all `already_applied` — receipt correctly empty), 13/13 complete, one audit only, no 📊 line. Cause (DEF-137): two seeded steps name non-`life:` ids, so their only id is the shared anchor role and the exact-id pass eats the wrong calls. Slice 6 (`codex/philote-shared-anchor-ids`): shared ids ignored in Pass 0 (live-fixture replay, batch == incremental); audit anchors only `life:` ids and flags the rest for judgment.
- [x] Slice 6 watched-live 2026-09-15 19:22 UTC: verification now correct (6/13 honest trailer → 12/13 → harness audit 62→63 → 13/13) but the continuation re-ran all 12 landed links and no 📊 line (DEF-138): continuation turns opened with statuses but no evidence flags ("1/13 verified, still outstanding: 1–12" in the duplicate-call guard), the re-entry envelope still offered 26 tools beside "[Plan complete]", and the trailer bailed on a skipped-duplicate audit entry. Slice 7 (`codex/philote-continuation-evidence`): `CarryoverPlan::seed_turn_plan` (plan + flags), `project_tools_for_turn` gate, trailer skips non-JSON entries.
- [x] Peer delegation forensics 2026-09-15 19:35 UTC (operator: "hand it off to bjork"): whisper failed honestly (hotel-local), `delegate.to_peer` acked "dispatched" twice, nothing reached mac-jane, Beacon claimed receipt. Three defects: DEF-139 node-less envelopes skipped by the ledger writer (phantom dispatch, fleet-wide); DEF-140 mac-jane's roster busts the 9216-byte Mac UDP datagram under the legacy encoding (no peer knows Björk); DEF-141 receipt claims over a "dispatched" ack. `codex/peer-delegation-routing`: resolve-or-refuse before acking; base64 beacon default; honest ack + 📨 trailer + delivery-claim detector.
- [x] DEF-140 proven live 2026-09-16 02:10 UTC (vps hotel log: mac-jane roster sync every 30 s after the base64 default; heartbeats had always arrived — the earlier "no heartbeats" reading was a journald artifact, the hotel's own tracing lives in `/opt/philotic/data/logs`). Follow-up: `delegate.to_peer` resolves peer agents from the gossiped rosters when the local graph has no identity (`codex/peer-delegation-registry`).
- [x] 2026-09-16 morning: ten `life.observe` refusals (stringified `properties`, DEF-144) + a read-only turn that promised tracking. Fixed on `codex/lifegraph-lenient-properties` (lenient properties on the runner, schema note, say-do read-only rule + promise patterns). Operator items to re-tell Beacon after deploy: pay bills (done 09-16), the Apple News article, buy Dr. Squatch deodorant, cancel Bronze David Bars, tonight's organ practice.
- [x] Re-sent items watched-live 2026-09-16 18:16 UTC: all six `life.observe` calls landed with ids (DEF-144/146 fixes proven). The same turn's refused whisper was reported to the operator as "routed to Björk" (DEF-150); fixed on `codex/philote-failed-step-agreement-2`. Data to correct with Beacon: an Event "Jared paid his outstanding bills" (`life:open_loop:pay_bills_event_20260916`) was written although the operator's message said "I need to pay my bills".
- [x] Peer delegation watched-live 2026-09-16 18:40 UTC: Beacon called `delegate.to_peer`, reply carried the 📨 "not confirmed" trailer, and the envelope reached mac-jane in one second — then was dropped ("no subscriber for role 'agent-bjork-01'", DEF-151). Fixed on `codex/peer-delegate-inbound-routing`.
- [x] Beacon review 2026-09-16 19:04–19:39 UTC (proposed-items skill + daily cron): the skill registered (`lifegraph.gardening-review`, validated, but `implied_tools: []`); the cron was undeliverable (DEF-152, plus two older dead jobs `069f52a1` workout_reminder and `1a0c3dea` whisper_practice_update); the "all proposed items" dashboard showed 15 of 197 (DEF-153); the review plan looped four times (DEF-154). Not code: she told the operator to run a nonexistent `/patch apply …` command and rendered fabricated confirm links; "where am i?" was answered with an invented location; "tell me the ENTIRE skill" embellished the record. Fixed on `codex/beacon-skill-cron-review`. Owed: re-register the gardening cron (and decide on the two dead jobs); the Beacon→Björk delegation test (DEF-151) has not been run.
- [ ] Watched-live: after deploy, ask Beacon to hand a task to Björk — expect either a refusal naming the unroutable agent (until mac-jane's roster syncs) or a "queued, not confirmed" reply, and a `peer.delegate` inbound on mac-jane; vps journal shows "Hotel state sync from mac-jane".
- [ ] Watched-live (slice 7): "Garden the LifeGraph" ends turn 1 at 12/13 with "still working on step 13 (Re-run life.audit…)", the continuation's harness closer runs `life.audit`, `plan_eval complete` 13/13, and the reply carries a before/after health score.
- [ ] **Operator direction 2026-09-12: no more operator-run Cypher patch scripts.** The philotes must keep the LifeGraph pristine themselves via a skill that organizes, checks integrity and connectedness, and uses graph science (duplicate/orphan detection, components, centrality, temporal consistency, label/edge conformance). `scripts/lifegraph-patches/2026-09-12-iphone-daxton-hygiene.cypher` is NOT to be run by hand; it becomes the first test case for the steward's hygiene pass. Next slice: `lifegraph.gardener` skill + runner-side `life.audit` (read-only report) and `life.tidy` (governed, resolve/retire/link — never delete) tools.
- [ ] Watched-live: next 11:00 UTC Beacon brief = at most 2 messages ("Working on it" + brief, or brief alone), replayed turns stamped `[YYYY-MM-DD HH:MM EDT]` in the model request, an explicit `life.recall` returns packets (journal: "dispatching as current_prompt_semantic").
...
## Bjork Repertoire Gardener — Spawn Gate + Claim-Over-Failure (2026-09-14)

Branch `codex/bjork-spawn-gate`. Source: bjork's Telegram session on mac-jane, 2026-09-14 14:42–14:43 EDT (`session_event` ledger + the distill review of that turn). Operator asked for the repertoire-gardener skill to be built first so the LifeGraph update would go through it safely.

- [x] Forensics: skill registered + assigned ✓; `subagent.spawn` with it → `SUBAGENT_FORBIDDEN` (DEF-126); first `life.observe.batch` rejected wholesale (no `source_refs`), retry wrote 8 `MusicSection` nodes; reply said the harness was used (DEF-127). Live graph: the 8 sections hang `RELATES_TO` OpenLoops (four of them off one 2026-08-30 La Campanella loop), none `HAS_SECTION` to their `CreativeWork` parent as the applied `patch:music_repertoire_ontology_v3` intended; key/tempo/difficulty live only in `claim_summary` prose.
- [x] DEF-126: `is_agent_handoff_caller` at the four agent-only IPC gates; role-incarnation spawn test + tool-identity refusal test.
- [x] DEF-127: `tool_result_is_error` payload marker; `unretried_failed_tools` / `reply_claims_over_failed_steps` / `FailedStepTrailer` in the say-do gate; 7 say-do tests from the live text + 3 detector tests.
- [x] Deployed all three hotels 2026-09-14 (mac-jane 16:19 EDT, mbp-jane 16:30 EDT, vps-jane 20:38 UTC; `Claim check` literal verified in every installed philote).
- [x] LifeGraph re-link DONE by bjork directly at 19:03 UTC after the operator's correction + `/approve` (two `life.observe.batch` calls): all 8 `MusicSection` nodes now `HAS_SECTION` from their `CreativeWork`. Typed key/tempo/difficulty still prose (no contract slot).
- [x] Watched-live 20:51 UTC: DEF-126 PROVEN — `subagent.spawn` from the orchestrator incarnation succeeded twice (guests 33c4395a…, 185bc884… materialized, registered, leases accepted). Found DEF-128: the workers idled — nothing ever assigns the delegation (philote never calls `AssignSubagentTask`); and DEF-129: `philote-worker` has no tool loop, so the gardener could not have run anyway.
...
- [ ] DEF-129: worker turn loop with `allowed_tools` projection — proposal/seam, not a patch. Until then a philote that delegates writes gets prose back. 2026-09-15 13:20 UTC: bjork's morning spawn got its delegation on lease accept (DEF-128 ✓), worker model leg timed out at 120 s again, failure hook fired, heal-dispatcher resolved `timeout → noop`; nothing surfaced to the operator.
- [x] DEF-130 (`codex/plan-eval-read-steps`): read-bound plan steps verify from the tool's result or from a successful call of the bound read tool — the "stops before the final step" pattern (today 13:21 + 13:28 UTC: `life.list` ×3 and `skill.list` ×2 never verified → stalls → `blocked`). Watched-live: any bjork turn whose last step is a `life.list`/`skill.list` read ends with `plan_eval verdict=complete` and no "Plan stopped before finishing" trailer.
- [ ] Today's other gap (not code): asked at 13:28 UTC to "make sure the skill is creating all these things and that you or your subagent are able to use it", bjork only listed the catalog and declared it ready — she never exercised the skill. With DEF-129 open she cannot; the honest reply is "the worker cannot run tools yet".
- [x] Operator test 14:14 UTC (DEF-130 build): "Check whether the Chopin nocturne is in my repertoire and confirm what sections it has. Plan it, run it, and report." → `plan_ready` → `life.list` → factual reply → `plan_eval verdict=complete` 1/1, no stop trailer. **DEF-130 watched-live-green.**
- [x] DEF-132 (`codex/handoff-home-node`): 14:44 UTC "Handed off to role 'theoretician'" went to a mesh peer named `mac-jane` (bare-name `home_node` vs node_id compare); fixed at both routing sites + IPC test; live record patched. Watched-live: next Telegram message reaches the theoretician locally (or bjork), no silence.
- [ ] DEF-133: `rule.propose` re-ran after `/approve` → duplicate rule (removed by hand). Fix: approval-completed step verified from the approval result.
- [ ] Operator's actual 14:42 ask — update `music.repertoire-gardener` so adding a piece also maps its sections — was NOT done: bjork proposed a rule instead (twice) and handed off. The skill's goal template still says nothing about sections; needs `skill.register`/patch of the template (and DEF-129 before a worker can run it).
- [x] DEF-132 watched-live 14:55 UTC (operator-chat handoff to the theoretician delivered locally, returned in 2 s). Operator's 16:05 UTC Telegram message reached the theoretician; `handoff.back` then failed → DEF-134 (bare role-name registration for mesh-config-seeded role guests) + DEF-135 (failure envelope reported as "unexpected hotel response"), both fixed on `codex/role-inbox-default`; session reset to the orchestrator by DB patch.
- [ ] 16:06 UTC the theoretician "updated the skill" by `life.patch.propose` of a `SkillPatch` node in the LifeGraph (`patch:repertoire_gardener_v2`) — the wrong plane: the hotel skill catalog (`abstract_skill music.repertoire-gardener`) is unchanged, no `skill.register`/audit today. The reply presented an invented skill JSON as "the updated definition". The DEF-127 claim gate DID catch the failed `handoff.back` (re-entry + correction trailer). Still owed: a real catalog update of the gardener's goal template (and the LifeGraph SkillPatch should be retired as a stray).
- [ ] DEFECTS.md has two `DEF-130` rows (another session used the id for "Apple agent selection…") — renumber the later one.
- [x] DEF-134/135 watched-live 16:23 UTC (handoff delivered to `role:agent-bjork-01:theoretician`, hand-back to `role:agent-bjork-01:orchestrator` in 1 s). The theoretician posture has `skill.list` + LifeGraph patch tools but no `skill.register` — which is why the 16:06 "skill update" went to the wrong plane.
- [x] 16:35 UTC operator-chat attempt to have bjork re-register the gardener: `skill.register` → REGISTER_FORBIDDEN (the base persona in an operator-chat session is not the orchestrator/management guest; the Telegram thread on the orchestrator incarnation is). Found DEF-136 (refusal text verified the step) + DEF-137 (promise "I will register this now" over the failure), fixed on `codex/claim-gate-promise`.
...
- [ ] **LifeGraph pristine (operator ask 2026-09-17: "make sure the sweep can automatically take care of these missteps")** — four layers: (1) write-time duplicate guard on `life.observe` — DONE, `codex/observe-duplicate-guard` (DEF-158); (2) audit findings the sweep can act on — DONE, same branch (DEF-159): `closable` = contradicted-by-completion + past-dated → `life.tidy resolve` in `suggested_actions`, and confirmed twins surface in `needs_judgment`; (3) bridging — DONE, `codex/lifegraph-bridging` (DEF-163): observe writes the ontology-validated edges a claim earns to the people/places/works it names and reports the rest; orphans anchor to the category Role they name. Still open in this layer: an audit `unbridged` finding so the sweep weaves EXISTING nodes, not just incoming ones; (4) two-lane sweep: auto-apply link/retire_duplicate/resolve, and hand anything touching a CONFIRMED node to the operator as one line per item with a reference number; (4) proposed-backlog pass (166 proposed nodes as of 09-17) grouped by type with counts. Note: the false-"retired" claim class is DEF-149 (owned elsewhere).

## Mesh Protocol Review Follow-ups (operator ask 2026-09-19: "make sure the interhotel protocol is optimal")

Source: three read-only reviews of the inter-hotel mesh (transport code, live ledger measurements, protocol semantics) on `origin/develop` @ `be6168aa`; the highest-severity claims were re-verified in code. Findings are DEF-181..190 in `docs/DEFECTS.md`. Proposed order — each step independently shippable, and the first two are wire-compatible with un-upgraded hotels:

- [ ] **A. Stop the bleeding (correctness, small):** *(measured 2026-09-19: steady-state volume is only ~20 events/h and a simple cross-hotel turn is 2 events + 2 acks (~4-5 KB), so bandwidth is not the problem — the nonce stores (DEF-185: call the dead sweeper, in-memory window, VACUUM 274-411 MB sidecars), the ~10 s stalls that cause 1.7x duplicate sends (DEF-191 — **root-caused and fixed 2026-09-19: NOT the receive loop; the heal-dispatcher's 30 s silence sweep called `list_session_turns` once per active session and each call full-scanned every `session_turn`, 438 sessions x ~24 ms ~= 10.5 s under the shared graph mutex; now index-backed, PR `codex/def-191-session-turn-scan`, verify live**; siblings DEF-194 unindexed `session_event` scans in `memory.report`, DEF-195 sessions never age out), and mac's roster at 87% of the datagram cap (DEF-192) are the urgent ones)* DEF-181 timeouts + per-target isolation + ACK off the inbound loop + max frame/read timeout/`accept` resilience; DEF-182 tolerant `EventKind`/per-event parse + ack-only-what-was-delivered + dead-letter + `attempt`/`expires_at`; DEF-183 UDP-plane sender binding + third-node target guard + `session.handoff` gate; DEF-184 `None` target fans out (turns mesh-coordinated cron on — check what it then does before enabling: it has never run on the wire).
- [ ] **B. Speed (latency/bandwidth):** DEF-185 `Notify` wake on append, batch per frame, shared nonce window + pruning timer, `(target_node_id, seq)` index; DEF-186 coalesce/supersede `partial_reply` for a remote seat + priority for final replies; DEF-188 change-driven roster sync (relocation SWITCH latency).
- [ ] **C. Protocol v2 (needs a rollout plan):** heartbeat `protocol_version` + `features` (retire ad-hoc `supports_continuity`); DEF-187 header-covering MAC with per-direction subkeys + `key_id` (dual-accept); term-based placement homes closing DEF-171; DEF-190 `corr_id`/`trace` + `phil mesh`; newtype ids (`NodeId`/`HotelName`/`GuestId`/`AgentId`); write `docs/architecture/MESH_PROTOCOL_SPEC.md` (authority table, compatibility contract, delivery contract, interval/TTL table with TTL ≥ 3 × interval, conformance tests).

## New Project: Decisions Model (System One judge)

Proposal: [docs/architecture/DECISIONS_MODEL_PROPOSAL.md](/Users/jaredlikes/code/philotic-stack/docs/architecture/DECISIONS_MODEL_PROPOSAL.md). **D0-D2 MERGED to develop 2026-09-23** (PR #574, `cb257a68`; egress-inventory follow-up PR #587, `c986a4e6`). D2 is LIVE on vps-jane (shadow, log-only; `PHILOTIC_SHADOW_DECISIONS=1` via `philotic_shadow_decisions_enabled` in `ansible/host_vars/jane-vps.yml`, the OpenRouter vault key already grants `model.decisions` + `heal-dispatcher`). Seam IDs: `decisions-envelope`, `decisions-shadow-heal`. Branch (closed, merged): `codex/decisions-model`.

A typed, calibrated decision capability (yes/no, choose-one, score) beside generate/transform/embed. First provider: TypeSafe Jev, native `/v1/systemone` or OpenRouter alpha `/api/alpha/decisions` (neither is chat completions). Shadow-first; never an authorization authority.

- [x] Research 2026-09-19: what Jev is, the two transports, the candidate decision points in the stack, and the request/response envelope — recorded in the proposal.
- [x] **Access confirmed 2026-09-19:** an ordinary OpenRouter key reaches Jev with no waitlist (`~typesafe/jev-latest` or pinned `typesafe/jev-1.13`; the bare `typesafe/jev-latest` slug returns 400). One probe: `noul` 0.95, 0.29-0.52 s. The native TypeSafe waitlist is optional. Smokes should use a dedicated key, and the operator's pasted test key should be rotated.
- [ ] **Operator: confirm or amend the data policy.** A recommendation is written in the proposal ("Data policy"): class A system telemetry only, sites allow-listed by id, a redaction gate on every string (the DEF-089 token shape included), default off, content-free audit rows; classes B and C excluded. Nothing may be enabled live until this is confirmed.
- [x] D0 `decisions-envelope` (2026-09-19, test-green): `ansible-mesh-core/src/decisions.rs` — canonical request/answer/trace/error types, `validate`, `build_wire_request` / `parse_wire_response` for both transports, `classify_http_status`. 27 unit tests; clippy clean. Limits follow the vendor docs: 64 k tokens per request and 32 k for `state` plus the longest question. A `score` legend mismatch is flagged (`legend_mismatch`), not fatal. **Before D1 trusts it: record one real `choice` + `score` body with a dedicated key.** `serde_json` `preserve_order` is NOT enabled (`cargo tree -e features`), so wire order comes from typed serializers, with an order test. Fixtures in `crates/ansible-mesh-core/tests/fixtures/decisions/`: only the OpenRouter `noul` response is recorded live; `choice`/`score` bodies are docs-derived until D1's smoke records real ones.
- [x] D1 model-router code (2026-09-19, test-green): `TaskKind::Decide`, `RequestClass::Judgment`, `ProviderOutput::Judgment`, `AuxTaskKind` arm, aux-isolation entry, `model_oracle` request-class mapping, `providers::DecisionsProvider` (native / OpenRouter), `model-controller-decisions` bin (role `model.decisions`), `ResponseTrace` transport/latency/usage, and `decisions_response` handled by `model-router/src/decisions.rs` from the raw task kind, before any generic handling. 23 new tests; full `model-router` (201) and `ansible-mesh-core` (417) lib suites pass; both crates clippy-clean.
- [x] **Live smoke with a dedicated key (2026-09-21):** one synthetic sentence with a `noul`, a `choice` and a `score` returned HTTP 200 in 0.51 s; the real `DecisionsClient` did the same in 609 ms (`#[ignore]`d `live_smoke` test). Real `choice` and `score` bodies are recorded as fixtures and the `score` legend assumption held (no mismatch). Not yet tested through the IPC controller (not seeded in a hotel).
- [x] **D2 key access decided (operator, 2026-09-21): option (b), a vault role grant, using the EXISTING `openrouter` vault key.** An earlier cut built a separate `decisions` vault entry; the operator preferred reusing the key already in the vault, so that was removed. The `openrouter` `ProviderKeySpec` now lists `model.decisions` and `heal-dispatcher` beside `model` and `model.openrouter`, and `decisions-client` reads `openrouter_api_key_ref` / `openrouter_base_url`. The model is never taken from `openrouter_default_model` (that is the chat model); it is the pinned `typesafe/jev-1.13` unless `PHILOTIC_DECISIONS_MODEL` says otherwise. **Trade-off accepted:** the chat key and the decisions calls share one spend, one rate limit and one revocation.
- [x] **Role grant for the entry already in the vault:** the sealed entry lists only `model` and `model.openrouter`, and a role list is fixed when it is stored. `aiua auth sync-roles --provider openrouter --db <hotel context db> [--dry-run]` adds the spec's roles to an existing entry **without decrypting it or re-entering the key** (ciphertext byte-identical, a test asserts it). Additive only; it refuses an empty role list (which means "any role", so adding one would restrict it) and a guest-restricted entry. Run it once per hotel that runs the pilot; the vault registry does not replicate.
- [x] **D2 prep done:** the HTTP hop and the key loader moved into a new small crate `crates/decisions-client` (`DecisionsClient`, `load_decisions_config`) that both `model-router` and `heal-dispatcher` can use. `model-router`'s `DecisionsProvider` is now a thin wrapper. D1's `decisions_response` still has no consumer until D3.
- [ ] D1 hotel wiring (operator, not needed for D2): seed the `model-controller-decisions-01` guest for role `model.decisions` (`crates/aiua/src/main.rs`, near `model-controller-openrouter`; hot file, check `just workstream-overlap` first). D2 is in-process and does not use the controller, so this waits for D3.
- [x] D2 `decisions-shadow-heal` code (2026-09-21, test-green): `heal-dispatcher/src/shadow.rs` — a log-only judge beside `gemma3:4b` for novel failure lines (rules run first and are not shadowed). Fire-and-forget (spawned, 4 in flight, 4 s deadline, no retry; a burst is skipped, not queued), so it cannot slow the poll loop. Asks `severity` (choice) and `needs_restart` (noul). Writes one content-free `decision_traces` row per call (`ansible-mesh-core/src/decision_trace.rs`): site, data class, bytes sent, resolved model, latency, usage, cost, `legend_mismatch`, the incumbent's verdict, the judge's answers, per-question agreement, and the **error class as its own column**; `summarize()` keeps errors out of agreement. The data-policy gate is at the client's single egress point: an allow-list of sites (only `heal.classify` and `smoke.live`, both class A), a fail-closed screen that refuses state resembling a conversation payload (`policy_refused`, recorded as a `skipped` row), and redaction of every string in `state` (the DEF-089 bot-token shape included). The pilot also never sends model-controller failure lines (a provider error body can echo the request) and asks only about the last 500 characters. `bytes_sent` is measured at the wire. `heal-dispatcher --decision-summary` prints errors, skips and agreement on separate lines. **Residual risk is not zero; see the proposal.** Off unless `PHILOTIC_SHADOW_DECISIONS` is set and the dedicated key loads. Tests: 26 in `decisions-client` (gate, redaction, egress), 9 in `shadow.rs` (including an end-to-end run against a local socket and a hung provider), 4 for the trace store.
- [x] **Data policy confirmed by the operator (2026-09-21).**
- [x] **D2 live on vps-jane (2026-09-23).** Operator chose option (B): ran `phil keys configure openrouter` on the vps themselves (creates the entry with `model.decisions` + `heal-dispatcher` already on `allowed_roles`, since it's a fresh entry — no `sync-roles` needed there). Flag applied durably via `ansible/host_vars/jane-vps.yml` (`philotic_shadow_decisions_enabled: true`) + a new `philotic-hotel.service.j2` conditional block, not a raw DB patch (survives future hotel restarts). Found and fixed along the way: `deploy_hotel.yml --tags config` matched zero tasks (no task carried that tag, despite the playbook's own header comment); the vault-loading `pre_task` also needed `tags: always` since any `--tags` filter skips untagged tasks. `ansible/vault/*.yml` is not in git — a worktree needs `cp -r` from the main checkout. **A concurrent session's routine develop deploy silently overwrote the branch-specific binaries mid-rollout** (confirmed via `strings <binary> | grep <marker>` and mtime) — the reason this got merged to develop rather than staying an unmerged branch. **Not yet checked:** whether `decision_traces.db` has real rows — read with `heal-dispatcher --decision-summary` on vps-jane.
- [ ] **Rotate the vault master key and mesh PSK** — the operator pasted the full decrypted contents of `ansible/vault/jane-vps.yml` into a session transcript while debugging (2026-09-23). Includes `vault_philotic_vault_master_key` (encrypts every secret in every hotel's context graph) and `vault_philotic_mesh_psk`, plus Beacon's Telegram token and the Gemini/ElevenLabs keys and Muninn credentials. Master-key rotation needs a real re-encryption migration across 3 hotels — no such tool exists yet. Deferred by the operator ("we can work through that later").
- [ ] D3 in-process client + local fallback; shadow `memory.recall` reflex relevance (`philote/src/mcp_handling.rs:166`) and the say-do gate (`philote/src/turn_loop.rs:2915`), non-blocking.
