...
- [x] D0 `decisions-envelope` (2026-09-19, test-green): `ansible-mesh-core/src/decisions.rs` — canonical request/answer/trace/error types, `validate`, `build_wire_request` / `parse_wire_response` for both transports, `classify_http_status`. 26 unit tests; clippy clean. `serde_json` `preserve_order` is NOT enabled (`cargo tree -e features`), so wire order comes from typed serializers, with an order test. Fixtures in `crates/ansible-mesh-core/tests/fixtures/decisions/`: only the OpenRouter `noul` response is recorded live; `choice`/`score` bodies are docs-derived until D1's smoke records real ones.
...
- [ ] D2 `decisions-shadow-heal`: `heal-dispatcher` classify calls the provider beside `gemma3:4b`, log-only, `decision_traces` rows, `PHILOTIC_SHADOW_DECISIONS` default off. Watched-live on one hotel.
