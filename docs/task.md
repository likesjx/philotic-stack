...
- [ ] D2 `decisions-shadow-heal`: `heal-dispatcher` classify calls the provider in-process beside `gemma3:4b`, log-only, `decision_traces` rows (record error class, resolved model and `legend_mismatch` as their own columns; never fold errors into "disagreement"), `PHILOTIC_SHADOW_DECISIONS` default off. Watched-live on one hotel.
