...
Proposal: [docs/architecture/DECISIONS_MODEL_PROPOSAL.md](/Users/jaredlikes/code/philotic-stack/docs/architecture/DECISIONS_MODEL_PROPOSAL.md) (proposed 2026-09-19, research only — nothing implemented). Seam IDs: `decisions-envelope`, `decisions-shadow-heal`. Branch: `codex/decisions-model`.
...
- [ ] [ASYNC] D0 `decisions-envelope`: `ansible-mesh-core::decisions` types + validation + native/OpenRouter wire adapters as pure functions with recorded fixtures; ordered-levels test; check `serde_json` `preserve_order` with `cargo tree -e features`. test-green.
