...
- [ ] **Phase 2 (2026-09-16 audit)** — philote memory efficiency, sleep, repair. See proposal § Phase 2. Sequence M1 → M2 → M3 (one PR rebased on develop) → M4 → M5 → M6 → M7.
  - [ ] **M1** recall query + relevance gate: drop `role:` prefix (one memory dominates 93% of Beacon recalls), follow-up/continuation seeding, honour `relevance_band`, 0–5 items, skip empty/failing vaults, partial-401 heal.
  - [ ] **M2** projection budget: slim item format, ~300-char preamble, fix ns timestamps, drop duplicate annotations + superseded items, LifeGraph own lane, stable prefix for prompt caching (today 97–99.5% of blocks truncated and re-sent on 73–89% of later calls).
  - [ ] **M3** engine hygiene + honest telemetry: shared engine, 1.5 s timeout, `memory_auto_recall_failed` event, band/score/latency in events, fix `memory.report` hit rate (reads ~100% vs real 91.6%), access feedback to Muninn.
  - [ ] **M4** write routing: every philote write + token mint targets the Cortex via `memory.write_forward` (Mac Attend-hook writes get 421 today; last Björk memory 07-11); durable retry queue (S2 remainder).
  - [ ] **M5** remember volume/quality: replace keyword gate on `memory.remember`, wire S3 capture after M4, collision-safe concept slugs, keep probes/self-heal out of persona vaults.
  - [ ] **M6** sleep on the Cortex hotel only: contradictions, consolidate (fix `dream.rs` vault discovery), expire dated events, enrichment backfill, tombstone purge, cross-node diff alert, `--metrics-addr` on all nodes. Never the `muninn dream` CLI.
