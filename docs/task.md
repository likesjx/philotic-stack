...
- [ ] D1 model-router: `TaskKind::Decide` (`decisions.evaluate`), `RequestClass::Judgment`, `ProviderOutput::Judgment`, `AuxTaskKind` arm, add to `isolate_aux_failure_from_cognitive_ladder`, `model_oracle` seeding, transport-switching provider, `model-controller-decisions` bin (role `model.decisions`), dedicated `decisions_response` reply action (never `model_response`), `ResponseTrace` usage/cost/latency.
