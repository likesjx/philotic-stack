...
- [x] Close the `bash.exec` egress bypass (shell egress fence): compiled-in `exec_guard::detect_network_egress` detector + `McpEgressPolicy` decision at both raw-shell dispatch points (tool-runner, philote runtime); loopback/tailnet allowed, all else denied and redirected to `http:<binding>.request`; `PHILOTIC_SHELL_EGRESS_ALLOW` env widening. Verified: test-green (exec-guard net_egress suite + tool-runner boundary tests). `Slice: codex/governed-egress-skilldag`.
- [x] Plug governed egress into the SkillDAG: seed a validated `net.http` skill (pure eligibility gate, implies no tools) and assign it to every philote toolset profile so bindings may gate on `grant_skills: ["net.http"]`. Scoped-eligibility (necessary-not-sufficient with `grant_agents` + host allowlist + approval); no blanket binding seeded. Reconciler unions it into deployed profiles on next boot. Verified: test-green (`net_http_egress_skill_is_seeded_and_held_by_every_philote_profile`). Seam: `skilldag-egress-capability`.
- [ ] Unify the shell egress allowlist into a hotel config node alongside `mcp_egress_policy` (retire the transitional `PHILOTIC_SHELL_EGRESS_ALLOW` env var); give operators a first-class place to register `net.http`-gated bindings. Seam: `shell-egress-fence`.
