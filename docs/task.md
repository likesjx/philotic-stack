...
- [ ] **Operator: data policy.** Which sites may send which state to a hosted judge, and whether OpenRouter is acceptable as an extra intermediary (ZDR is enterprise-only at TypeSafe). Default until decided: system telemetry only, no operator message or LifeGraph content.
...
- [ ] D1 smoke (needs the operator): (a) seed the `model-controller-decisions-01` guest for role `model.decisions` in the hotel (`crates/aiua/src/main.rs`, near `model-controller-openrouter`; hot file, check `just workstream-overlap` first); (b) re-run `phil keys configure openrouter` so the sealed key's `allowed_roles` picks up `model.decisions`; (c) a **dedicated** OpenRouter key (not the one pasted into the 2026-09-19 transcript); (d) one live multi-question call, then record real `choice` and `score` response bodies as fixtures and confirm or correct the `score` legend assumption.
- [ ] **Operator: D2 key access.** How does `heal-dispatcher` get a key: (a) env var on its unit, (b) vault role grant, or (c) via the `model.decisions` controller over IPC (needs an inbox and a correlator)? See "D2 shape" in the proposal. Recommendation: (a) for the shadow pilot.
- [ ] D2 prep: move `DecisionsProvider::evaluate` (the HTTP hop) out of `model-router` into a small shared crate so `heal-dispatcher` can call it without depending on `model-router`. D1's `decisions_response` has no consumer until D3.
