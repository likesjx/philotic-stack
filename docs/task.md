...
- [ ] **Operator: confirm access.** Jev is early-access/waitlisted; unknown whether OpenRouter's alpha endpoint is waitlist-gated. Try an OpenRouter key against `/api/alpha/decisions` (the hotel already holds an OpenRouter key) or join the TypeSafe waitlist. Gates every smoke below.
