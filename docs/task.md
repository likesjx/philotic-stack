...
- [x] **D2 key access decided (operator, 2026-09-21): option (b), a vault role grant.** Built as a **dedicated** `decisions` vault entry rather than widening the chat OpenRouter key: `ProviderKeySpec` `decisions` (`decisions_api_key`), readable only by roles `model.decisions` and `heal-dispatcher`; the `openrouter` key's role list is unchanged. `phil keys configure decisions` stores it (the operator runs this and pastes the key at the prompt; defaults are the OpenRouter base URL and the pinned `typesafe/jev-1.13`).
...
- [ ] **D2 watched-live (operator-gated):** needs (1) the data policy confirmed, (2) `phil keys configure decisions` run so the vault entry exists with roles `model.decisions` + `heal-dispatcher`, (3) `PHILOTIC_SHADOW_DECISIONS=1` on the `heal-dispatcher` unit of ONE hotel, and (4) a heal-dispatcher restart. Then let it run and read the rows with `heal-dispatcher --decision-summary` (errors by class, skips by reason, legend mismatches, per-question agreement, bytes sent). Nothing is enabled by default.
