...
- [ ] Add a session-shaped native-live provider seam under `model-router` for Gemini 3.1 Flash Live style execution, covering Live API connection lifecycle, streamed PCM audio I/O, sequential tool-response turns, and resumable session markers instead of pretending `ModelProvider::invoke` is already the right shape.
