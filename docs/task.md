...
- [ ] **D2 watched-live (operator-gated):** needs (1) the data policy confirmed, (2) `aiua auth sync-roles --provider openrouter --db <that hotel's context db> --dry-run`, then again without `--dry-run`, so the existing OpenRouter entry lists `model.decisions` + `heal-dispatcher`, (3) `PHILOTIC_SHADOW_DECISIONS=1` on the `heal-dispatcher` unit of ONE hotel, and (4) a heal-dispatcher restart. Then let it run and read the rows with `heal-dispatcher --decision-summary` (errors by class, skips by reason, legend mismatches, per-question agreement, bytes sent). Nothing is enabled by default.
