...
- [ ] Replace the remaining trigger-class heuristics for remembered role handoff with role-manifest, skill-marker, and toolset-marker receptors so role shifts are learned from declared scope and abilities rather than only implementation/research substring biochemistry.
