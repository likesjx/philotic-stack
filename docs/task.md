...
- [ ] **LifeGraph pristine (operator ask 2026-09-17: "make sure the sweep can automatically take care of these missteps")** — four layers: (1) write-time duplicate guard on `life.observe` — DONE, `codex/observe-duplicate-guard` (DEF-158); (2) audit findings the sweep can act on: a live loop contradicted by a completion record for the same subject, and past-dated loops/events → suggest `resolve`; (3) two-lane sweep: auto-apply link/retire_duplicate/resolve, and hand anything touching a CONFIRMED node to the operator as one line per item with a reference number; (4) proposed-backlog pass (166 proposed nodes as of 09-17) grouped by type with counts. Note: the false-"retired" claim class is DEF-149 (owned elsewhere).
