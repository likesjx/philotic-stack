...
Proposal: [docs/architecture/DECISIONS_MODEL_PROPOSAL.md](/Users/jaredlikes/code/philotic-stack/docs/architecture/DECISIONS_MODEL_PROPOSAL.md) (proposed 2026-09-19; D0 envelope types implemented, nothing wired into a runtime yet). Seam IDs: `decisions-envelope`, `decisions-shadow-heal`. Branch: `codex/decisions-model`.
...
- [ ] **D2 watched-live, BLOCKED on where the key is (2026-09-21).** The operator asked for vps-jane, and read-only inspection found **vps-jane's vault has no OpenRouter key** (no `openrouter_api_key_ref`, no key env on the hotel or its OpenRouter controller; the vault registry does not replicate). The key is in **mac-jane's** vault (legacy kind `vault-token`, roles `model` + `model.openrouter`, no guest restriction). Nothing on the vps was changed. Choose one: (A) run the pilot on mac-jane (`sync-roles` works there; needs a macOS build and a mac deploy); (B) put an OpenRouter key into the vps vault (`phil keys configure openrouter` on the vps; note this ALSO turns on the currently keyless vps `model.openrouter` controller/fallback tier, and a key used only by the pilot would avoid that); (C) a `PHILOTIC_OPENROUTER_API_KEY` env override on the vps (not durable and puts the key in unit config). Then, on the chosen hotel: `aiua auth sync-roles --provider openrouter --db <that hotel's context db> --dry-run`, then again without `--dry-run`; `PHILOTIC_SHADOW_DECISIONS=1` (and `PHILOTIC_DECISION_TRACE_DB=<writable path>`) in the `heal-dispatcher` guest's `env`, which the hotel re-reads at respawn (it is re-seeded, so wiped, at every hotel start); bounce the `heal-dispatcher` process. The vps also needs a Linux artifact built from this branch (`gh workflow run build-linux.yml --ref codex/decisions-model`; `vps-deploy-ci` only ships develop) and a targeted install of `heal-dispatcher`. Then let it run and read the rows with `heal-dispatcher --decision-summary` (errors by class, skips by reason, legend mismatches, per-question agreement, bytes sent). Nothing is enabled by default.
