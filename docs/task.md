...
- [ ] **Phase 2 (2026-09-16 audit)** — philote memory efficiency, sleep, repair. See proposal § Phase 2. Sequence M1 → M2 → M3 (one PR rebased on develop) → M4 → M5 → M6 → M7.
  ...
  - [ ] **Follow-up from M7**: a checkpoint restore replaces the observer's auth store with the Cortex's, so each Mac hotel's stored Muninn admin credential (`config:muninn`, plaintext) no longer logs in — `aiua load` fails its Muninn admin login (401; `push-homebrew-remote.sh` exits 1 though the hotel runs). Set both Macs' credential to the Cortex's via the encrypted `muninn_admin_secret_ref`, and remove the plaintext copy.
