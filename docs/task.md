...
- [ ] Deploy mac-jane (bjork) and vps-jane; verify the `SUBAGENT_FORBIDDEN` gate string and `[Claim check]` literal in the installed binaries.
- [ ] Watched-live: operator tells bjork the spawn was refused and asks her to run `music.repertoire-gardener` to re-link the eight sections `HAS_SECTION` to their pieces → expect `subagent.spawn` ok, a `philote-worker` doing the writes, and a reply that reports what the worker wrote.
