...
- [ ] **Attachment evidence + batch ids (2026-09-17, `codex/media-evidence-carry`)** — operator review of bjork's morning ("Fuck! You didn't update the hymns from that image"). DEF-160 re-entries carry the refused draft so an attachment's reading survives the say-do gate; DEF-161 a caption-less attachment with an open operator request re-enters as INPUT instead of being described back; DEF-162 batch items get their ids synthesized. Watched-live gates: (a) a photo + instruction caption whose first reply is refused still writes what the photo says, not a recalled older value; (b) a caption-less photo after "send me X again" performs X; (c) a model-authored `life.observe.batch` with no packet_id lands on the FIRST attempt. Left over from the same review: duplicate practice logs for 09-16 night to be tidied by bjork (Beacon's `life:event:music_practice_20260916_tonight` vs bjork's `organ_practice_20260916_hymns` + `piano_practice_20260916_classical`); `properties.kind`/`notes` still undeclared for CreativeWork/Event (R1 typed properties needs the music keys declared); agent-coach's `life.recall` hung 301 s (watchdog evicted); Muninn token heal 421 for vault `user_likesjx` degrades recall.
