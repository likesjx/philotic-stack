...
TMP_DIR="$(mktemp -d)"
HOTEL_NAME="jane-smoke-deny-redir-$$"
...
export PHILOTIC_MODEL_ROUTER_STUB_RESPONSE="approval-turn-1=APPROVAL_REQUIRED: deploy the thing;approval-turn-1:2=Redirected: summarize the plan instead"
...
cleanup() {
  ...
  if [[ ${exit_code} -ne 0 ]]; then
    echo "Deny-redirect smoke failed. aiua log:"
