...
cleanup() {
  ...
  if [[ ${exit_code} -ne 0 ]]; then
    echo "Preapprove smoke failed. ansible log:"
    [[ -f "${TMP_DIR}/ansible.log" ]] && cat "${TMP_DIR}/ansible.log"
    echo "Preapprove smoke failed. agent log:"
    [[ -f "${TMP_DIR}/agent.log" ]] && cat "${TMP_DIR}/agent.log"
    echo "Preapprove smoke failed. model log:"
...
echo "Building preapprove-smoke binaries..."
cargo build -p ansible -p agent-core -p model-router -p philotic-client --example preapprove_smoke_driver >/dev/null
...
echo "Driving preapprove round-trip..."
PHILOTIC_HOTEL_SOCKET="${SOCKET_PATH}" \
  cargo run -q -p philotic-client --example preapprove_smoke_driver

echo "Preapprove smoke round-trip succeeded."
