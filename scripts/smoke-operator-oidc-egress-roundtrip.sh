...
TMP_DIR="$(mktemp -d)"
HOTEL_NAME="operator-oidc-smoke-$$"
...
cleanup() {
  ...
  if [[ ${exit_code} -ne 0 ]]; then
    echo "Operator OIDC egress smoke failed. aiua log:"
    [[ -f "${TMP_DIR}/aiua.log" ]] && sed -n '1,240p' "${TMP_DIR}/aiua.log"
    echo "Operator OIDC egress smoke failed. runner log:"
...
echo "Building operator OIDC governed-egress smoke binaries..."
cargo build -p aiua -p egress-http-runner >/dev/null
cargo build -p philotic-client --example operator_oidc_egress_smoke_driver >/dev/null

AIUA_BIN="${ROOT_DIR}/target/debug/aiua"
RUNNER_BIN="${ROOT_DIR}/target/debug/egress-http-runner"

echo "Starting isolated smoke hotel..."
(
  cd "${TMP_DIR}"
...
echo "Starting local credential-bearing egress runner..."
PHILOTIC_HOTEL_SOCKET="${SOCKET_PATH}" \
...
echo "Driving IPC, vault, token, userinfo, sanitized response, and audit round-trip..."
PHILOTIC_HOTEL_SOCKET="${SOCKET_PATH}" \
cargo run -q -p philotic-client --example operator_oidc_egress_smoke_driver

echo "Operator OIDC governed-egress smoke round-trip succeeded."
