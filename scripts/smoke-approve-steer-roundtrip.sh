...
TMP_DIR="$(mktemp -d)"
HOTEL_NAME="jane-smoke-appr-steer-$$"
...
export PHILOTIC_MODEL_ROUTER_STUB_RESPONSE="approval-turn-1=APPROVAL_REQUIRED: deploy the thing;approval-turn-1:2=Steered approval acknowledged: use staging"
...
cleanup() {
  ...
  if [[ ${exit_code} -ne 0 ]]; then
    echo "Approve-steer smoke failed. ansible log:"
    [[ -f "${TMP_DIR}/ansible.log" ]] && cat "${TMP_DIR}/ansible.log"
    echo "Approve-steer smoke failed. agent log:"
    [[ -f "${TMP_DIR}/agent.log" ]] && cat "${TMP_DIR}/agent.log"
    echo "Approve-steer smoke failed. model log:"
...
echo "Building approve-steer smoke binaries..."
...
echo "Driving approve-steer round-trip..."
PHILOTIC_HOTEL_SOCKET="${SOCKET_PATH}" \
PHILOTIC_SMOKE_APPROVAL_COMMAND="/approve use staging" \
PHILOTIC_SMOKE_EXPECTED_FINAL="Steered approval acknowledged: use staging" \
  cargo run -q -p philotic-client --example approval_smoke_driver

echo "Approve-steer smoke round-trip succeeded."
