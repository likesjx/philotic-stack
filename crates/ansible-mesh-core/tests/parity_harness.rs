...
fn mock_event(id: uuid::Uuid, seq: u64) -> EventEnvelope {
    EventEnvelope {
        ...
        expires_at: None,
        payload: EventPayload::Inline { data: r#"{"hello":"world"}"#.to_string() },
        trace: vec![],
    }
}
...
async fn test_authz_crypto_validation_and_time_bound() {
    ...
    assert!(auth.validate(&msg_id, seq, payload, now, &valid_hmac).is_ok(), "Valid HMAC should pass");
    
    // 2. Tampered Payload (Forgery)
    let bad_payload = b"tampered_payload_data";
    assert!(auth.validate(&msg_id, seq, bad_payload, now, &valid_hmac).is_err(), "Invalid payload must fail HMAC");
    ...
    assert!(auth.validate(&msg_id, seq, payload, old_timestamp, &old_hmac).is_err(), "Expired timestamp must fail time window checks");
}
...
fn test_nonce_replay_guard() {
    ...
    assert!(tracker.assert_and_record_nonce(&msg_id).is_ok(), "Fresh nonce must be accepted");
    ...
    assert!(err_str.contains("Replay detected"), 
        "Error message string should contain replay detection wrapper indication: {}", err_str);
}
...
fn test_ledger_idempotent_deduplication() {
    ...
    ledger.append_event(&mut evt1.clone()).expect("Append event 1");
    ledger.append_event(&mut evt2.clone()).expect("Append event 2");
    
    // Check count
    let count: u64 = ledger.conn.lock().unwrap().query_row("SELECT count(*) FROM mesh_events", [], |row| row.get(0)).unwrap();
    ...
    let count_after: u64 = ledger.conn.lock().unwrap().query_row("SELECT count(*) FROM mesh_events", [], |row| row.get(0)).unwrap();
    assert_eq!(count_after, 2, "Ledger idempotency failed: duplicate events were stored!");
}
...
fn test_cursor_advancement() {
    ...
    // 1. Uninitialized
    assert_eq!(tracker.get_cursor(consumer).unwrap(), 0, "Default cursor should be 0");
    
    // 2. Advance
    let ts = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_millis() as u64;
    tracker.advance_cursor(consumer, 10, ts).expect("Advance cursor to 10");
    assert_eq!(tracker.get_cursor(consumer).unwrap(), 10, "Cursor should be 10");
    
    // 3. Advance further
    tracker.advance_cursor(consumer, 25, ts).expect("Advance cursor to 25");
    assert_eq!(tracker.get_cursor(consumer).unwrap(), 25, "Cursor should be 25");
}
