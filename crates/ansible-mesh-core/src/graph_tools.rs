...
impl ContextGraphInvoker {
    ...
    async fn write_memory(&self, args: Value) -> Result<Value> {
        ...
        let agent_id = args.get("agent_id").and_then(|v| v.as_str()).unwrap_or("unknown");
        let entry: MemoryEntry = serde_json::from_value(args.get("entry").cloned().unwrap_or_default())?;
        
        let mut lock = self.apartments.write().await;
        let apt = lock.entry(agent_id.to_string()).or_insert_with(|| MemoryApartment {
