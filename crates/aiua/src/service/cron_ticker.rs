...
use ansible_mesh_core::cron::{
    CronInterpolationVars, CronJob, CronJobSource, interpolate_payload, next_fire_after,
};
...
impl CronTicker {
    ...
    async fn fire(&self, job: &CronJob, now_ms: u64) {
        ...
        // Scripted hotel sensors (`doc:scripted-hotel-sensors`): same
        // in-process sentinel interception as memory.hygiene/dream-sweep,
        // but the payload names which locally-registered script to run
        // rather than the job itself doing fixed work. No opt-in flag: a
        // hotel with no matching `sensor_script:<id>` config row simply has
        // nothing to run, so mesh-replicated job definitions from a peer
        // hotel are naturally inert here.
        if job.target_role == crate::sensor_scripts::CRON_TARGET_ROLE {
            self.fire_sensor_script(job, fire_epoch, now_ms).await;
            if let Err(e) = self.advance_schedule(job, fire_epoch) {
                error!(
                    "CronTicker: sensor_script advance failed for job {}: {e}",
                    job.id
                );
            }
            return;
    ...
    /// Run the sensor script named in `job.payload`'s `sensor_id`
    /// in-process, and dispatch its verdict. `Deliver` reuses the exact same
    /// `TaskInvoke`-envelope + role-inbox delivery path every other cron job
    /// uses ([`build_cron_task_json`] promotes `message` into routable
    /// content) — a sensor's dispatch looks identical to any other cron
    /// delivery to the receiving agent. `Investigate` is not yet wired (see
    /// `sensor_scripts` module docs) and is logged, not dropped silently.
    async fn fire_sensor_script(&self, job: &CronJob, fire_epoch: u64, now_ms: u64) {
        let sensor_id = serde_json::from_str::<serde_json::Value>(&job.payload)
            .ok()
            .and_then(|v| {
                v.get("sensor_id")
                    .and_then(|s| s.as_str())
        ...
        let Some(sensor_id) = sensor_id else {
            warn!(
                job_id = %job.id,
                payload = %job.payload,
                "CronTicker: sensor_script job fired with no sensor_id in payload — skipping"
            );
            return;
        };

        let verdict = match crate::sensor_scripts::evaluate(&self.graph, &sensor_id) {
            Ok(v) => v,
            Err(e) => {
                error!(
                    sensor_id,
                    "CronTicker: sensor_script evaluation failed: {e}"
                );
                return;
            }
        };
        ...
        match verdict {
            None | Some(crate::sensor_scripts::SensorVerdict::Quiet) => {
                debug!(sensor_id, "CronTicker: sensor_script quiet tick");
            }
            Some(crate::sensor_scripts::SensorVerdict::Deliver {
                target_role,
                message,
            }) => {
                self.deliver_sensor_message(&job.id, &fire_epoch, &target_role, message, now_ms)
                    .await;
            }
            Some(crate::sensor_scripts::SensorVerdict::Investigate { target_role, brief }) => {
                warn!(
                    sensor_id,
                    target_role,
                    brief,
                    "CronTicker: sensor_script returned Investigate, which has no dispatch \
                     path yet (doc:scripted-hotel-sensors) — dropped, not delivered"
                );
            }
        }
    }

    /// Materialize and deliver a `TaskInvoke` for a sensor's `Deliver`
    /// verdict — the non-guaranteed subset of the tail of [`Self::fire`]
    /// (claim → append to ledger → resolve role → deliver-or-park), reused
    /// here because the target role and payload are decided by the script at
    /// run time rather than being the job's own static fields.
    async fn deliver_sensor_message(
        &self,
        source_job_id: &str,
        fire_epoch: &u64,
        target_role: &str,
        ...
        let payload_data = serde_json::json!({ "message": message }).to_string();
        let synthetic_job = CronJob {
            id: format!("{source_job_id}:sensor-dispatch"),
            schedule: String::new(),
            ...
            next_fire_at: *fire_epoch,
            created_at: now_ms,
            created_by: CronJobSource::Guest("sensor-scripts".to_string()),
            silent_ok: false,
            session_target: ansible_mesh_core::cron::CronSessionTarget::Main,
        };
        let task_json = build_cron_task_json(
            &synthetic_job,
            *fire_epoch,
            &self.local_node_id,
            payload_data,
        ...
        if !crate::service::ipc::claim_delivery(&self.delivery_claims, task_id) {
            warn!(
                "CronTicker: event id {} for sensor dispatch (job {}) already claimed; skipping",
        ...
        let task_env = EventEnvelope {
            ...
            target_node_id: Some(self.local_node_id.clone()),
            source_agent_id: "cron-ticker".into(),
            target_agent_id: Some(target_role.to_string()),
            kind: EventKind::TaskInvoke,
            corr_id: format!("cron:{source_job_id}"),
            attempt: 0,
            created_at: now_ms,
            expires_at: None,
            payload: EventPayload::Inline {
            ...
            trace: vec![format!("cron-ticker:sensor:{source_job_id}")],
        };
        ...
        {
            error!(
                "CronTicker: failed to append sensor dispatch TaskInvoke for job {}: {e}",
                source_job_id
            );
            return;
        }

        let target_role_record = Self::resolve_target_role_record(&self.graph, target_role);
        let is_subscribed = {
            let guard = self.inboxes.lock().await;
            let role_subs = guard.get(target_role).cloned().unwrap_or_default();
            match &target_role_record {
                Some(record) => role_subs.iter().any(|s| s.guest_id == record.guest_id),
                None => !role_subs.is_empty(),
            }
        ...
        info!(
            "CronTicker: sensor dispatch delivered → job={} role={}",
            source_job_id, target_role
        );
    }
...
mod tests {
    ...
    // ── Scripted hotel sensors (`doc:scripted-hotel-sensors`) ───────────────

    fn sensor_job(job_id: &str, sensor_id: &str, next_fire_at: u64) -> CronJob {
        CronJob {
            id: job_id.to_string(),
            schedule: "0 */5 * * * * *".to_string(),
            target_role: crate::sensor_scripts::CRON_TARGET_ROLE.to_string(),
            target_node_id: None,
            payload: serde_json::json!({ "sensor_id": sensor_id }).to_string(),
            guaranteed: false,
            enabled: true,
            last_fired_epoch: None,
            next_fire_at,
    ...
    /// No `sensor_script:<id>` registered locally → quiet skip, schedule
    /// still advances, nothing reaches the ledger. This is the natural
    /// per-hotel gate: a `CronJobSync`-replicated sensor job on a peer hotel
    /// that never registered the matching script does nothing here.
    #[tokio::test]
    async fn sensor_fire_is_quiet_when_no_script_registered() {
        use ansible_mesh_core::sqlite_storage::SqliteGraphStorage;
        ...
        let job = sensor_job("sensor:unregistered", "does-not-exist", 1_000);
        graph.upsert_cron_job(&job).expect("seed job");
        ...
        let after = graph
            ...
            .expect("lookup")
            .expect("job still present");
        assert!(after.next_fire_at > 1_000, "schedule must still advance");
        assert!(parked_inbound.lock().await.is_empty());
        assert!(dispatcher_rx.try_recv().is_err());
    }

    /// A registered, enabled, operator-approved script that calls `deliver`
    /// produces exactly one `TaskInvoke` append targeting the verdict's
    /// `target_role` — not the job's own sentinel `target_role`.
    #[tokio::test]
    async fn sensor_fire_delivers_when_script_calls_deliver() {
        use ansible_mesh_core::sqlite_storage::SqliteGraphStorage;
        ...
        let script = crate::sensor_scripts::SensorScript {
            id: "always-fires".to_string(),
            source: r#"deliver("role:agent-beacon:orchestrator", "test payload")"#.to_string(),
            enabled: true,
            operator_approved: true,
            last_run_at: None,
            last_result: None,
        };
        script.save(&graph).expect("save script");

        let job = sensor_job("sensor:always-fires", "always-fires", 1_000);
        graph.upsert_cron_job(&job).expect("seed job");
        ...
        // `test_dispatcher_channel` forwards through a spawned task (bounded
        // -> unbounded relay), so the message may not have landed the instant
        // `fire()` returns — await it rather than racing with `try_recv`.
        let received = tokio::time::timeout(Duration::from_secs(2), dispatcher_rx.recv())
            .await
            .expect("must not hang waiting for the sensor's TaskInvoke")
            .expect("sensor's Deliver verdict must append a TaskInvoke");
        ...
        assert_eq!(
            ...
            "dispatch must target the verdict's role, not the sensor sentinel"
        );
        assert!(
            match &envelope.payload {
                EventPayload::Inline { data } => data.contains("test payload"),
                _ => false,
            },
            "delivered message must be present in the task payload"
        );
        ...
        // last_result was persisted for observability.
        let saved = crate::sensor_scripts::SensorScript::load(&graph, "always-fires")
            .expect("lookup")
            .expect("script still present");
        assert_eq!(saved.last_result.as_deref(), Some("delivered"));
