...
pub async fn run(
    ...
    let app = Router::new()
        ...
        // Embedded UI — same-origin cookie auth bootstrap; all other assets served as-is
...
async fn handle_apartment(
    ...
    (
        ...
        Json(json!({
            "error": format!(
                "apartment inspection for [{}] is not exposed by the desktop membrane",
                agent_id
            ),
            "code": "APARTMENT_VIEW_DISABLED"
        })),
    )
        .into_response()
}
