...
impl Materializer for LocalProcessMaterializer {
    async fn spawn_guest(&mut self, guest_id: &str, config_json: &serde_json::Value) -> Result<String> {
        if let Some(cmd) = config_json.get("command").and_then(|c| c.as_str()) {
            ...
            let child = command.spawn().context("Failed to spawn OS child process")?;
    ...
    async fn reclaim_guest(&mut self, guest_id: &str) -> Result<()> {
        if let Some(mut child) = self.children.remove(guest_id) {
            ...
            match child.start_kill() {
                Ok(()) => info!("Forcefully retired tracked OS Process PID {} for Guest [{}].", pid, guest_id),
                Err(e) => warn!("Failed to start termination for tracked Guest [{}] PID {}: {}", guest_id, pid, e),
            }
    ...
    async fn check_status(&mut self, guest_id: &str, active_id: &str) -> Result<bool> {
        if let Some(child) = self.children.get_mut(guest_id) {
            match child.try_wait().context("Failed to query tracked child status")? {
                None => return Ok(true),
                Some(status) => {
                    warn!(
...
impl GuestManager {
    pub fn new(hotel_name: impl Into<String>, graph: Arc<dyn GraphStorage>, materializer: Box<dyn Materializer>) -> Self {
        Self {
        ...
        for rec in guest_records {
            ...
            match mat.spawn_guest(&rec.guest_id, &config).await {
                Ok(child_pid) => {
                    ...
                    let _ = self.graph.set_guest_pid(&self.hotel_name, &rec.guest_id, Some(&child_pid));
                }
                Err(e) => {
                    Self::clear_guest_pid(self.graph.as_ref(), &self.hotel_name, &rec.guest_id);
                ...
                if rec.active_pid.is_none() {
                    ...
                    match mat.spawn_guest(&rec.guest_id, &config).await {
                        Ok(new_pid) => {
                            ...
                            let _ = self.graph.set_guest_pid(&self.hotel_name, &rec.guest_id, Some(&new_pid));
                        }
...
mod tests {
    ...
    impl Materializer for MockMaterializer {
        async fn spawn_guest(&mut self, _guest_id: &str, _config_json: &serde_json::Value) -> Result<String> {
    ...
    async fn reconcile_all_does_not_respawn_healthy_active_guest() {
        ...
        let graph: Arc<dyn GraphStorage> = Arc::new(TestGraphStorage::with_guests(vec![GuestRecord {
        ...
        manager.reconcile_all().await.expect("reconcile should succeed");
